...
def init_db():
    ...
    db.execute("UPDATE users SET department_id=? WHERE department_id IS NULL AND role='teacher'", (default_dept,))
...
def login():
    ...
    if request.method == "POST":
        ...
        if user and user["role"] == "hod":
            flash("This account type is no longer available. Please contact the administrator.", "error")
            return redirect(url_for("login"))
...
def student_register():
    if request.method == "POST":
        ...
        context = dict(year_options=YEAR_OPTIONS, selected_year=year, security_questions=SECURITY_QUESTIONS)
        db_for_depts = get_db(); _dept_row = db_for_depts.execute("SELECT id FROM departments ORDER BY id LIMIT 1").fetchone(); db_for_depts.close()
        department_id = _dept_row["id"] if _dept_row else None
    ...
    return render_template("student_register.html", year_options=YEAR_OPTIONS, selected_year="", security_questions=SECURITY_QUESTIONS)
...
def add_student():
    db = get_db()
    try:
        # Department is hidden from the website, but the database still keeps
        # one internal department record for existing attendance relationships.
        dept_row = db.execute("SELECT id FROM departments WHERE active=1 ORDER BY id LIMIT 1").fetchone()
        if not dept_row:
        ...
        if request.method == "POST":
            prn = request.form.get("prn", "").strip()
            name = request.form.get("name", "").strip()
            course = request.form.get("course", "").strip() or "B.Pharm"
            year = request.form.get("year", "").strip()

            if not prn or not name or year not in YEAR_OPTIONS:
                flash("PRN, name and valid year are required.", "error")
                return render_template("student_form.html", student=None, title="Add Student", year_options=YEAR_OPTIONS)

            if db.execute("SELECT id FROM students WHERE prn=?", (prn,)).fetchone():
                flash("PRN already exists. Please use a different PRN.", "error")
                return render_template("student_form.html", student=None, title="Add Student", year_options=YEAR_OPTIONS)

            db.execute("INSERT INTO students(roll_no,prn,name,course,year,division,department_id) VALUES(?,?,?,?,?,?,?)",
                       (prn, prn, name, course, year, "A", dept_id))
            db.commit()
        ...
        try:
            ...
            pass
        flash(f"Student could not be added: {exc}", "error")
        return render_template("student_form.html", student=None, title="Add Student", year_options=YEAR_OPTIONS)
    finally:
        try:
            db.close()
...
def edit_student(student_id):
    ...
    if request.method == "POST":
        ...
        if not data[0] or not data[1] or data[3] not in YEAR_OPTIONS:
            ...
            try:
                # Department is no longer shown in the UI, so keep the student's
                # existing internal department_id instead of expecting a form value.
                dept_id = student["department_id"]
                if not dept_id:
                    _dept_row = db.execute("SELECT id FROM departments WHERE active=1 ORDER BY id LIMIT 1").fetchone()
                    dept_id = _dept_row["id"] if _dept_row else None
                if not dept_id:
                    raise ValueError("No active internal department record is available.")

                db.execute("UPDATE students SET roll_no=?,prn=?,name=?,course=?,year=?,department_id=? WHERE id=?",
                           (data[0], data[0], data[1], data[2] or "B.Pharm", data[3], dept_id, student_id))
    ...
    db.close(); return render_template("student_form.html", student=student, title="Edit Student", year_options=YEAR_OPTIONS)
...
def subjects():
    db = get_db()
    rows = db.execute("SELECT s.* FROM subjects s ORDER BY s.year, s.code").fetchall()
    db.close()
...
def add_subject():
    ...
    if request.method == "POST":
        ...
        year = request.form.get("year", "").strip(); _dept_row = db.execute("SELECT id FROM departments ORDER BY id LIMIT 1").fetchone(); dept_id = _dept_row["id"] if _dept_row else None
        ...
        if not code or not name or not year or not dept_id:
            ...
            return render_template("subject_form.html", subject=None, title="Add Subject", departments=[], years=YEAR_OPTIONS)
        try:
    ...
    db.close(); return render_template("subject_form.html", subject=None, title="Add Subject", departments=[], years=YEAR_OPTIONS)
...
def edit_subject(subject_id):
    ...
    if request.method == "POST":
        code = request.form.get("code", "").strip().upper(); name = request.form.get("name", "").strip(); year = request.form.get("year", "").strip(); _dept_row = db.execute("SELECT id FROM departments ORDER BY id LIMIT 1").fetchone(); dept_id = _dept_row["id"] if _dept_row else None
        if not code or not name or year not in YEAR_OPTIONS or not dept_id:
            ...
            try:
                ...
                if not isinstance(exc, sqlite3.IntegrityError) and not (POSTGRES_AVAILABLE and isinstance(exc, psycopg2.IntegrityError)): raise
                flash("Subject code already exists.", "error")
    db.close(); return render_template("subject_form.html", subject=subject, title="Edit Subject", departments=[], years=YEAR_OPTIONS)
