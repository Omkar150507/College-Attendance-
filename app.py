...
def timetable():
    ...
    if request.method=='POST':
        ...
        if year not in YEAR_OPTIONS or day not in days or not start or not end or start >= end or not subject_id or not teacher_id or session_type not in ('Lecture','Practical'):
            ...
            # Legacy department_id is kept only for database compatibility; it is never shown in the UI.
            dept_row=db.execute('SELECT id FROM departments ORDER BY id LIMIT 1').fetchone()
            dept=dept_row['id'] if dept_row else None
    ...
    if view not in ('day','week'): view='day'
    if selected_day not in days: selected_day=date.today().strftime('%A')
    if role=='student':
        st=current_student(db)
        selected_year=st['year'] if st else selected_year
    where,params=timetable_scope_sql(role,session.get('user_id'),current_student(db) if role=='student' else None)
    if selected_year and role in ('admin','teacher'): where.append('t.year=?'); params.append(selected_year)
    if view=='day':
        where.append('t.day_of_week=?'); params.append(selected_day)
    elif selected_day:
        # Week view intentionally ignores the selected day and displays all six working days.
        pass
    rows=db.execute(f"""SELECT t.*,COALESCE(t.session_type,'Lecture') session_type,COALESCE(t.batch,'') batch,
        ...
        FROM timetable t LEFT JOIN subjects sub ON sub.id=t.subject_id LEFT JOIN users u ON u.id=t.teacher_id
        WHERE {' AND '.join(where) if where else '1=1'} ORDER BY CASE t.day_of_week WHEN 'Monday' THEN 1 WHEN 'Tuesday' THEN 2 WHEN 'Wednesday' THEN 3 WHEN 'Thursday' THEN 4 WHEN 'Friday' THEN 5 WHEN 'Saturday' THEN 6 ELSE 7 END,t.start_time,t.year,t.id""",tuple(params)).fetchall()
    subjects=db.execute('SELECT id,code,name,year FROM subjects ORDER BY year,code').fetchall(); teachers=all_teacher_rows(db)
    db.close()
    return render_template('timetable.html',rows=rows,subjects=subjects,teachers=teachers,years=YEAR_OPTIONS,selected_year=selected_year,selected_day=selected_day,days=days,batch_options=batches,view=view)
...
def export_timetable_pdf():
    ...
    where,params=timetable_scope_sql(role,session.get('user_id'),student)
    if selected_year and role in ('admin','teacher'): where.append('t.year=?'); params.append(selected_year)
    if view=='day': where.append('t.day_of_week=?'); params.append(selected_day)
    rows=db.execute(f"""SELECT t.*,COALESCE(t.session_type,'Lecture') session_type,COALESCE(t.batch,'') batch,
        COALESCE(sub.code,'') subject_code,COALESCE(sub.name,'') subject_name,COALESCE(u.full_name,u.username,'') teacher_name
        FROM timetable t LEFT JOIN subjects sub ON sub.id=t.subject_id LEFT JOIN users u ON u.id=t.teacher_id
        WHERE {' AND '.join(where) if where else '1=1'} ORDER BY CASE t.day_of_week WHEN 'Monday' THEN 1 WHEN 'Tuesday' THEN 2 WHEN 'Wednesday' THEN 3 WHEN 'Thursday' THEN 4 WHEN 'Friday' THEN 5 WHEN 'Saturday' THEN 6 ELSE 7 END,t.start_time,t.year,t.id""",tuple(params)).fetchall()
    ...
    buffer=io.BytesIO()
    doc=SimpleDocTemplate(buffer,pagesize=landscape(A4),rightMargin=9*mm,leftMargin=9*mm,topMargin=9*mm,bottomMargin=12*mm)
    styles=getSampleStyleSheet()
    title=ParagraphStyle('TTTitle',parent=styles['Title'],alignment=TA_CENTER,fontSize=20,leading=23,textColor=colors.HexColor('#123b6b'))
    sub=ParagraphStyle('TTSub',parent=styles['Normal'],alignment=TA_CENTER,fontSize=11,leading=14,textColor=colors.HexColor('#1d66a8'))
    meta=ParagraphStyle('TTMeta',parent=styles['Normal'],alignment=TA_CENTER,fontSize=9,leading=12,textColor=colors.HexColor('#4f6478'))
    cell=ParagraphStyle('TTCell',parent=styles['Normal'],fontSize=7.7,leading=9, textColor=colors.HexColor('#263f55'))
    head=ParagraphStyle('TTHead',parent=cell,fontName='Helvetica-Bold',alignment=TA_CENTER,textColor=colors.white)
    story=[]
    logo_path=BASE_DIR/'static'/LOGO_FILE
    logo=Image(str(logo_path),width=22*mm,height=22*mm) if logo_path.exists() else ''
    scope_year=selected_year or (student['year'] if student else 'All Years')
    scope_title='Weekly Academic Timetable' if view=='week' else f'{selected_day} Timetable'
    header_text=[Paragraph(xml_escape(str(COLLEGE_NAME)),title),Paragraph(scope_title,sub),Paragraph('Affiliated by Dr. Babasaheb Ambedkar Technological University, Lonere, Raigad',meta),Paragraph(f'<b>Course:</b> B.Pharm &nbsp;&nbsp; <b>Year:</b> {xml_escape(scope_year)}',meta)]
    hdr=Table([[logo,header_text]],colWidths=[27*mm,240*mm])
    hdr.setStyle(TableStyle([('VALIGN',(0,0),(-1,-1),'MIDDLE'),('ALIGN',(1,0),(1,0),'CENTER'),('LEFTPADDING',(0,0),(-1,-1),0),('RIGHTPADDING',(0,0),(-1,-1),0),('TOPPADDING',(0,0),(-1,-1),0),('BOTTOMPADDING',(0,0),(-1,-1),0)]))
    ...
    if view=='day':
        ...
        for r in rows:
            subj=f"{r['subject_code']} — {r['subject_name']}" if r['subject_code'] else (r['subject_name'] or '—')
            data.append([Paragraph(str(r['year'] or '—'),cell),Paragraph(f"<b>{r['start_time']}</b> – <b>{r['end_time']}</b>",cell),Paragraph(str(r['session_type'] or 'Lecture'),cell),Paragraph(r['batch'] or 'All',cell),Paragraph(xml_escape(subj),cell),Paragraph(xml_escape(r['teacher_name'] or '—'),cell),Paragraph(xml_escape(r['room'] or '—'),cell)])
        if not rows: data.append([Paragraph('No timetable entries found for the selected day.',cell),'','','','','',''])
        table=Table(data,repeatRows=1,colWidths=[23*mm,34*mm,27*mm,25*mm,62*mm,48*mm,48*mm])
    else:
        data=[[Paragraph('Day',head),Paragraph('Time',head),Paragraph('Year',head),Paragraph('Session',head),Paragraph('Batch',head),Paragraph('Subject',head),Paragraph('Teacher',head),Paragraph('Room / Lab',head)]]
        for r in rows:
            subj=f"{r['subject_code']} — {r['subject_name']}" if r['subject_code'] else (r['subject_name'] or '—')
            data.append([Paragraph(str(r['day_of_week']),cell),Paragraph(f"{r['start_time']} – {r['end_time']}",cell),Paragraph(str(r['year'] or '—'),cell),Paragraph(str(r['session_type'] or 'Lecture'),cell),Paragraph(r['batch'] or 'All',cell),Paragraph(xml_escape(subj),cell),Paragraph(xml_escape(r['teacher_name'] or '—'),cell),Paragraph(xml_escape(r['room'] or '—'),cell)])
        if not rows: data.append([Paragraph('No timetable entries found.',cell),'','','','','','',''])
        table=Table(data,repeatRows=1,colWidths=[25*mm,31*mm,23*mm,25*mm,24*mm,57*mm,43*mm,39*mm])
    table.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.HexColor('#07579a')),('GRID',(0,0),(-1,-1),0.45,colors.HexColor('#a9c7df')),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('ROWBACKGROUNDS',(0,1),(-1,-1),[colors.white,colors.HexColor('#eef7ff')]),('TOPPADDING',(0,0),(-1,-1),5.5),('BOTTOMPADDING',(0,0),(-1,-1),5.5),('LEFTPADDING',(0,0),(-1,-1),5),('RIGHTPADDING',(0,0),(-1,-1),5)]))
    story += [table,Spacer(1,5*mm),Paragraph(f'<b>Generated On:</b> {datetime.now(ZoneInfo("Asia/Kolkata")).strftime("%d %b %Y | %I:%M %p")}',ParagraphStyle('TTGen',parent=meta,alignment=TA_LEFT,fontSize=8.5))]
    def footer(canvas,doc_obj):
        canvas.saveState(); w,h=landscape(A4); canvas.setFillColor(colors.HexColor('#084b83')); canvas.rect(0,0,w,8*mm,fill=1,stroke=0); canvas.setFillColor(colors.white); canvas.setFont('Helvetica',8.5); canvas.drawCentredString(w/2,2.8*mm,'Y.N.P. College of Pharmacy  •  Better Education  •  Brighter Future'); canvas.drawRightString(w-8*mm,2.8*mm,f'Page {doc_obj.page}'); canvas.restoreState()
    doc.build(story,onFirstPage=footer,onLaterPages=footer)
    pdf=buffer.getvalue(); buffer.close()
    suffix='week' if view=='week' else selected_day.lower()
    return Response(pdf,mimetype='application/pdf',headers={'Content-Disposition':f'attachment; filename=timetable_{scope_year.replace(" ","_")}_{suffix}.pdf'})
...
def qr_attendance():
    ...
    try:
        ...
        ad=date.today().isoformat(); day_name=date.today().strftime('%A')
    ...
    db.close(); return render_template('qr_attendance.html',subjects=subjects,years=years,attendance_date=ad,qr=qr,qr_session=qr_session,token=token,batch=batch,session_type=session_type,candidates=candidates,teachers=teachers,selected_year=selected_year,selected_subject=subject_id,day_name=day_name)
