...
def timetable():
    ...
    if role == "student":
        ...
        if not student:
            ...
            return redirect(url_for("login"))
        where.append("t.year = ?")
        params.append(student["year"])
    elif role == "teacher":
        where.append("t.teacher_id = ?")
        params.append(session.get("user_id"))
    query = """SELECT t.*, s.code AS subject_code, s.name AS subject_name,
    ...
    query += " ORDER BY CASE t.day_of_week WHEN 'Monday' THEN 1 WHEN 'Tuesday' THEN 2 WHEN 'Wednesday' THEN 3 WHEN 'Thursday' THEN 4 WHEN 'Friday' THEN 5 WHEN 'Saturday' THEN 6 ELSE 7 END, t.start_time"
    rows = db.execute(query, params).fetchall()
    db.close()
    return render_template("timetable.html", rows=rows, role=role)
