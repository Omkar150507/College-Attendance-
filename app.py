...
def timetable():
    ...
    db.close()
    return render_template('timetable.html',rows=rows,subjects=subjects,teachers=teachers,years=YEAR_OPTIONS,selected_year=selected_year,selected_day=selected_day,days=days,batch_options=batches,view=view,session_type=session_type,batch=batch,slots=slots)
...
def export_timetable_pdf():
    ...
    # Unique subject/teacher/room legend.
    legend=[]; seen=set()
    for r in rows:
        key=(r['subject_code'],r['subject_name'],r['teacher_name'],r['room'] or '')
        if key not in seen:
            seen.add(key); legend.append(key)
    db.close()
    ...
    header_lines=[Paragraph('V. M. P. PATH, KRUSHI PRATISHTHAN',institute),Paragraph(str(COLLEGE_NAME).upper(),title),Paragraph('Recognized by Govt. of Maharashtra',small),Paragraph('Affiliated by DBATU, Lonere, Raigad & Affiliated by MSBTE Mumbai',small),Paragraph('DTE CODE-3533 &nbsp;&nbsp;&nbsp;&nbsp; MSBTE CODE-12311 &nbsp;&nbsp;&nbsp;&nbsp; PCI CODE-6184',small),Paragraph('Off.: Asangaon, Station - Vangaon (Western Railway), Tal. Dahanu, Dist. Palghar - 401 103.',small),Paragraph('E-mail: ynpbpharm2022@gmail.com &nbsp;&nbsp; Website: www.ynpbpharm.in &nbsp;&nbsp; Mobile: 7249579956',small)]
    ...
    if view=='week':
        ...
        data=[header_row]
        # Map exact slot/day to entries.
        lookup={}
        for r in rows:
            lookup.setdefault((r['start_time'],r['end_time'],r['day_of_week']),[]).append(r)
        for idx,(st,en) in enumerate(slots):
            # Lunch break row for a long gap before this slot.
            if idx>0:
                prev=slots[idx-1]
                if _time_minutes(st)-_time_minutes(prev[1])>=30:
                    data.append([Paragraph('LUNCH BREAK',band)]+[Paragraph('LUNCH BREAK',band) for _ in days])
            cells=[Paragraph(f'<b>{_format_time_12(st)}</b> To <b>{_format_time_12(en)}</b>',cell_left)]
            for d in days:
                entries=lookup.get((st,en,d),[])
                bits=[]
                for r in entries:
                    subj=f"{r['subject_code']}" if r['subject_code'] else r['subject_name']
                    details=f"<b>{subj}</b>"
                    if r['session_type']=='Practical': details += f" ({r['batch'] or 'Batch'})"
                    details += f"<br/>{r['teacher_name'] or '—'}"
                    if r['room']: details += f"<br/>{r['room']}"
                    bits.append(details)
        ...
        slots=_timetable_slot_rows(rows,days)
        for st,en in slots:
            entries=[r for r in rows if r['day_of_week']==selected_day and r['start_time']==st and r['end_time']==en]
            bits=[]
            for r in entries:
                subj=r['subject_code'] or r['subject_name']; txt=f"<b>{subj}</b>"
                if r['session_type']=='Practical': txt += f" ({r['batch'] or 'Batch'})"
                txt += f"<br/>{r['teacher_name'] or '—'}"
                if r['room']: txt += f"<br/>{r['room']}"
                bits.append(txt)
            data.append([Paragraph(f'<b>{_format_time_12(st)}</b> To <b>{_format_time_12(en)}</b>',cell_left),Paragraph('<br/><br/>'.join(bits) if bits else '—',cell)])
        table=Table(data,repeatRows=1,colWidths=[58*mm,206*mm])
    table.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.HexColor('#d9ecfb')),('BOX',(0,0),(-1,-1),0.65,colors.HexColor('#7ea8c9')),('INNERGRID',(0,0),(-1,-1),0.35,colors.HexColor('#9ab9d2')),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('TOPPADDING',(0,0),(-1,-1),4.3),('BOTTOMPADDING',(0,0),(-1,-1),4.3),('LEFTPADDING',(0,0),(-1,-1),4),('RIGHTPADDING',(0,0),(-1,-1),4)]))
    story += [table,Spacer(1,3.5*mm)]

    if legend:
        ldata=[[Paragraph('Subjects',head),Paragraph('Subject Teacher',head),Paragraph('Lab / Room',head)]]
        for code,name,teacher,room in legend:
            ldata.append([Paragraph(f'<b>{code or "—"}</b> : {name or "—"}',cell_left),Paragraph(teacher or '—',cell_left),Paragraph(room or '—',cell_left)])
        ...
        story.append(lt)
    story += [Spacer(1,2.5*mm),Paragraph(f'<b>W.e.f.:</b> {datetime.now(ZoneInfo("Asia/Kolkata")).strftime("%d/%m/%Y")}',note),Paragraph('<b>Note:</b> Practical sessions are batch-specific. Lecture sessions apply to the selected year. Timetable changes should be updated by the College Administrator.',note)]
...
def student_my_report():
    ...
    return render_template("student_report.html", student=student, summary=summary, history=history,
                           total=total, present=present, absent=absent, overall=overall, report_mode="student")
...
def student_calendar():
    ...
    if not student:
        db.close(); session.clear(); flash("Student account is not linked correctly. Contact the administrator.","error"); return redirect(url_for("login"))
    ...
    days=db.execute("""SELECT a.attendance_date,
        SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END) present,
        SUM(CASE WHEN a.status='Absent' THEN 1 ELSE 0 END) absent, COUNT(a.id) total
        FROM attendance a WHERE a.student_id=? AND a.attendance_date>=? AND a.attendance_date<?
        GROUP BY a.attendance_date ORDER BY a.attendance_date""",(student["id"],start,end)).fetchall()
    details=[]
    if selected_date and re.match(r"^\d{4}-\d{2}-\d{2}$",selected_date) and start <= selected_date < end:
        details=db.execute("""SELECT a.attendance_date, COALESCE(sub.code,'') code, COALESCE(sub.name,'') subject_name,
            COALESCE(a.lecture_no,1) lecture_no, COALESCE(a.lecture_time,'') lecture_time, a.status
            FROM attendance a LEFT JOIN subjects sub ON sub.id=a.subject_id
            WHERE a.student_id=? AND a.attendance_date=?
            ORDER BY a.lecture_no, sub.code, a.id""",(student["id"],selected_date)).fetchall()
    db.close(); return render_template("student_calendar.html",student=student,month=month,days=days,details=details,selected_date=selected_date)
...
@app.post("/activity-log/clear")
@login_required
@admin_required
def clear_activity_log():
    db=get_db(); mode=request.form.get("mode","all")
    if mode == "30":
        db.execute("DELETE FROM activity_logs WHERE created_at < datetime('now','-30 days')")
        msg="Activity older than 30 days cleared."
    else:
        db.execute("DELETE FROM activity_logs")
        msg="Activity log cleared."
    db.commit(); db.close(); flash(msg,"success"); return redirect(url_for("activity_log"))
...
def student_report(student_id):
    ...
    db.close(); return render_template("student_report.html", student=student, rows=rows, report_mode="staff")
...
def clear_notifications():
    db=get_db(); mode=request.form.get("mode","all")
    if mode == "30":
        db.execute("DELETE FROM notifications WHERE user_id=? AND created_at < datetime('now','-30 days')", (session["user_id"],))
        msg="Notifications older than 30 days cleared."
    else:
        db.execute("DELETE FROM notifications WHERE user_id=?", (session["user_id"],))
        msg="Notifications cleared."
    db.commit(); db.close(); flash(msg,"success"); return redirect(url_for("notifications"))
...
@app.post("/leaves/clear")
@login_required
def clear_leaves():
    db=get_db(); mode=request.form.get("mode","all"); role=session.get("role")
    if role == "student":
        st=current_student(db)
        if not st:
            db.close(); return redirect(url_for("login"))
        scope="student_id=?"; params=[st["id"]]
    elif role in ("admin","teacher"):
        scope="1=1"; params=[]
    else:
        db.close(); return redirect(url_for("leaves"))
    if mode == "30":
        db.execute(f"DELETE FROM leave_applications WHERE {scope} AND created_at < datetime('now','-30 days')", params)
        msg="Leave applications older than 30 days cleared."
    else:
        db.execute(f"DELETE FROM leave_applications WHERE {scope}", params)
        msg="Leave applications cleared."
    db.commit(); db.close(); flash(msg,"success"); return redirect(url_for("leaves"))
