...
def export_timetable_pdf():
    ...
    # Consolidated subject details: one row per subject + teacher, preferring the lab.
    legend=[]; seen={};
    for r in rows:
        key=(r['subject_code'],r['subject_name'],r['teacher_name'])
        if key not in seen:
            item=[r['subject_code'],r['subject_name'],r['teacher_name'],r['room'] or '',r['session_type']]
            seen[key]=item; legend.append(item)
        elif r['room'] and r['session_type']=='Practical':
            # Prefer the practical lab over a lecture room when the same subject/teacher is reused.
            seen[key][3]=r['room']
            seen[key][4]='Practical'
    db.close()
    ...
    if legend:
        ...
        for code,name,teacher,room,_session in legend:
