        ...
        if not data[0] or not data[1] or data[3] not in YEAR_OPTIONS:
            ...
            try:
                ...
                if duplicate:
                    ...
                    # Department is hidden from the website. Keep the existing
                    # internal department relationship for attendance/subjects.
                    ...
                    if not dept_id:
                        db.execute(
                            "INSERT INTO departments(code,name,active) VALUES(?,?,1)",
                            ("PHARM", "Pharmacy")
                        )
                        db.commit()
                        dept_id = db.execute(
                    ...
                    db.execute(
                        "UPDATE students SET roll_no=?,prn=?,name=?,course=?,year=?,department_id=? WHERE id=?",
                        (data[0], data[0], data[1], data[2] or "B.Pharm", data[3], dept_id, student_id)
                    )
                    db.execute(
                        "UPDATE users SET full_name=? WHERE student_id=? AND role='student'",
                        (data[1], student_id)
                    )
                    db.commit()
                ...
                try:
                    ...
                    pass
                flash(f"Student could not be updated: {exc}", "error")
    db.close()
    return render_template("student_form.html", student=student, title="Edit Student", year_options=YEAR_OPTIONS)
...
def export_pdf():
    ...
    for r in rows:
        data.append([
            r["prn"], r["name"], r["total"], r["present"],
            r["absent"], f'{r["pct"]}%'
        ])
