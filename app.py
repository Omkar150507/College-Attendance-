...
class CompatCursor:
    def __init__(self, cursor, postgres=False):
        ...
        self.lastrowid = None
        # psycopg2 cursors expose rowcount, but the compatibility wrapper
        # previously did not. Attendance-history deletion uses this value.
        ...
        if user and check_password_hash(user["password"], password):
            if user["approved"] == 0:
                if user["role"] == "teacher":
                    ...
                    message = "Your HOD account is waiting for admin approval."
                else:
                    message = "Your student account is waiting for approval."
                flash(message, "error")
                return redirect(url_for("login"))
            session.clear()
...
@app.route("/hod/register", methods=["GET", "POST"])
def hod_register():
    """Register an HOD account. New HOD accounts remain pending until Admin approval."""
    db = get_db()
    ...
    if request.method == "POST":
        ...
        if not all([
            full_name, username, department_id, email, mobile,
            security_question, security_answer, password, confirm
        ]):
            db.close()
            flash("All HOD registration fields are required.", "error")
            return render_template("hod_register.html", **context)
        ...
        if not department:
            db.close()
            flash("Please select a valid active department.", "error")
            return render_template("hod_register.html", **context)
        ...
        try:
            ...
            if db.execute(
                ...
                db.close()
                flash("This email is already linked to another account.", "error")
                return render_template("hod_register.html", **context)

            db.execute(
                """INSERT INTO users(
                    username,password,role,full_name,email,mobile,
                    security_question,security_answer,approved,department_id
                ) VALUES(?,?,?,?,?,?,?,?,0,?)""",
                (
                    username,
                    ...
                    mobile,
                    security_question,
                    generate_password_hash(security_answer.casefold()),
                    department_id,
                ),
            )
            db.commit()
            db.close()
            flash(
                "HOD registration submitted. Ask the Admin to approve your account.",
                "success",
            )
            return redirect(url_for("login"))
            ...
            if not isinstance(exc, sqlite3.IntegrityError) and not (
                POSTGRES_AVAILABLE and isinstance(exc, psycopg2.IntegrityError)
            ):
                db.rollback()
                db.close()
                raise
            db.rollback()
            db.close()
            flash("Username or email already exists. Please use different details.", "error")
...
def student_calendar():
    ...
    # Validate the month itself so values such as 2026-99 cannot produce
    # invalid date ranges.
    try:
    ...
    # One calendar cell per date. Include present + absent so the frontend
    # can show green/red/mixed days correctly.
    rows = db.execute(
        """SELECT
               a.attendance_date,
               SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END) present,
    ...
    # Detailed records for the selected date. This is what the new calendar
    # right-side panel uses to show Subject, Lecture, Time and Status.
    selected_date = request.args.get("date", "").strip()
    ...
    if selected_date:
        details = db.execute(
            """SELECT
                   ...
                   COALESCE(a.lecture_no,1) lecture_no,
                   COALESCE(a.lecture_time,'') lecture_time,
                   a.status
               FROM attendance a
               JOIN subjects sub ON sub.id=a.subject_id
               WHERE a.student_id=?
                 AND a.attendance_date=?
               ORDER BY sub.code, a.lecture_no""",
...
@app.route("/hod/<int:hod_id>/approve", methods=["POST"])
@login_required
@admin_required
def approve_hod(hod_id):
    db = get_db()
    hod = db.execute(
        "SELECT id,full_name,username FROM users WHERE id=? AND role='hod'",
        (hod_id,)
    ).fetchone()
    if not hod:
        db.close()
    ...
    db.execute("UPDATE users SET approved=1 WHERE id=? AND role='hod'", (hod_id,))
    db.commit()
    db.close()
    flash(f"HOD account {hod['full_name'] or hod['username']} approved.", "success")
    return redirect(url_for("departments"))


@app.route("/hod/<int:hod_id>/toggle", methods=["POST"])
@login_required
@admin_required
def toggle_hod(hod_id):
    db = get_db()
    db.execute(
        "UPDATE users SET approved=CASE approved WHEN 1 THEN 0 ELSE 1 END WHERE id=? AND role='hod'",
        (hod_id,),
    )
    db.commit()
    db.close()
    flash("HOD account status updated.", "success")
    return redirect(url_for("departments"))
