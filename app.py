...
def students():
    ...
    rows = db.execute(f"""SELECT s.*, u.username student_username
        ...
        {where}
        ORDER BY CASE WHEN s.prn GLOB '[0-9]*' THEN CAST(s.prn AS INTEGER) ELSE 999999999 END, s.prn, s.name""", tuple(params)).fetchall()
...
def take_attendance():
    ...
    if selected_subject and not can_use_subject(db, selected_subject): flash("You are not assigned to this subject.", "error"); selected_subject = 0
    if selected_subject:
        if session_type == "Practical":
            students = db.execute("SELECT * FROM students WHERE year=? AND batch=? ORDER BY CASE WHEN prn GLOB '[0-9]*' THEN CAST(prn AS INTEGER) ELSE 999999999 END, prn, name", (selected_year, selected_batch)).fetchall() if selected_batch else []
        else:
            students = db.execute("SELECT * FROM students WHERE year=? ORDER BY CASE WHEN prn GLOB '[0-9]*' THEN CAST(prn AS INTEGER) ELSE 999999999 END, prn, name", (selected_year,)).fetchall()
    else: students=[]
    lecture_times={}
    academic_row=db.execute("SELECT value FROM app_settings WHERE key='academic_year'").fetchone(); current_academic_year=(academic_row["value"] if academic_row and academic_row["value"] else "2026-27")
    if request.method=="POST" and selected_subject and students:
        for n in range(1,lecture_count+1): lecture_times[n]=request.form.get(f"lecture_time_{n}","").strip()
        for st in students:
            for n in range(1,lecture_count+1):
                status=request.form.get(f"status_{st['id']}_{n}")
                if status in ("Present","Absent"):
                    db.execute("""INSERT INTO attendance(student_id,subject_id,attendance_date,lecture_no,lecture_time,session_type,batch,status,marked_by,academic_year) VALUES(?,?,?,?,?,?,?,?,?,?)
                    ON CONFLICT(student_id,subject_id,attendance_date,lecture_no) DO UPDATE SET lecture_time=excluded.lecture_time,session_type=excluded.session_type,batch=excluded.batch,status=excluded.status,marked_by=excluded.marked_by,academic_year=excluded.academic_year""",(st["id"],selected_subject,selected_date,n,lecture_times[n],session_type,selected_batch,status,session["user_id"],current_academic_year))
        for n in range(1,lecture_count+1): notify_students_for_attendance(db,selected_subject,selected_year,selected_date,n)
        create_low_attendance_notifications(db); db.commit(); flash(f"{session_type} attendance saved for {lecture_count} lecture(s).","success")
    existing={}
    if selected_subject:
...
def qr_attendance():
    db=get_db(); subjects=teacher_or_admin_subjects(db)
    if request.method=='POST':
        subject_id=request.form.get('subject_id',type=int); year=request.form.get('year','').strip(); session_type=request.form.get('session_type','Lecture').strip(); batch=request.form.get('batch','').strip(); ad=request.form.get('attendance_date','').strip(); ln=request.form.get('lecture_no',type=int); lt=request.form.get('lecture_time','').strip()
        valid_subject=any(s['id']==subject_id and s['year']==year for s in subjects)
        if session_type not in ('Lecture','Practical') or not subject_id or not year or not ad or ln not in (1,2,3) or not valid_subject or not can_use_subject(db,subject_id): flash("Invalid QR attendance details.","error")
        elif session_type=='Practical' and batch not in ('Batch A','Batch B','Batch C','Batch D'): flash("Select a valid practical batch.","error")
        else:
            token=secrets.token_urlsafe(24); exp=datetime.utcnow().replace(microsecond=0)+__import__('datetime').timedelta(minutes=10)
            db.execute("INSERT INTO qr_sessions(token,subject_id,year,attendance_date,lecture_no,lecture_time,session_type,batch,teacher_id,expires_at,active) VALUES(?,?,?,?,?,?,?,?,?,?,1)",(token,subject_id,year,ad,ln,lt,session_type,batch if session_type=='Practical' else '',session['user_id'],exp.isoformat()))
    ...
    db.close(); return render_template('qr_attendance.html',subjects=subjects,years=YEAR_OPTIONS,token=token,qr=qr,qr_session=session_info,today=date.today().isoformat(),batch_options=['Batch A','Batch B','Batch C'])
