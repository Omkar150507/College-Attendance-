...
def student_report(student_id):
    # Staff may view any student's report; a student may view only their own report.
    if session.get("role") == "student":
        db = get_db(); own = current_student(db)
        if not own or own["id"] != student_id:
            db.close(); flash("You can view only your own attendance report.", "error"); return redirect(url_for("student_dashboard"))
    elif session.get("role") not in ("admin", "teacher", "hod"):
        flash("Login required.", "error"); return redirect(url_for("login"))
    db = get_db(); student = db.execute("SELECT * FROM students WHERE id=?", (student_id,)).fetchone()
    if not student:
        db.close(); flash("Student not found.", "error"); return redirect(url_for("student_dashboard") if session.get("role") == "student" else url_for("students"))
    rows = db.execute("""SELECT sub.code,sub.name subject_name,COUNT(a.id) total,
        COALESCE(SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END),0) present,
        COALESCE(SUM(CASE WHEN a.status='Absent' THEN 1 ELSE 0 END),0) absent,
        CASE WHEN COUNT(a.id)=0 THEN 0 ELSE ROUND(100.0*SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END)/COUNT(a.id),1) END pct
        FROM subjects sub LEFT JOIN attendance a ON a.subject_id=sub.id AND a.student_id=?
        WHERE sub.department_id=? GROUP BY sub.id ORDER BY sub.code""", (student_id, student["department_id"])).fetchall()
    total = sum(r["total"] for r in rows)
    present = sum(r["present"] for r in rows)
    absent = sum(r["absent"] for r in rows)
    overall = round((present / total * 100), 1) if total else 0
    db.close(); return render_template("student_report.html", student=student, rows=rows, total=total, present=present, absent=absent, overall=overall)
...
def student_my_report():
    ...
    rows = db.execute("""SELECT sub.code,sub.name subject_name,COUNT(a.id) total,
        ...
        FROM subjects sub LEFT JOIN attendance a ON a.subject_id=sub.id AND a.student_id=?
        WHERE sub.department_id=? GROUP BY sub.id ORDER BY sub.code""", (student_id, student["department_id"])).fetchall()
    total = sum(r["total"] for r in rows); present = sum(r["present"] for r in rows); absent = sum(r["absent"] for r in rows)
    overall = round((present / total * 100), 1) if total else 0
    db.close(); return render_template("student_report.html", student=student, rows=rows, total=total, present=present, absent=absent, overall=overall)
...
def student_my_report_pdf():
    db = get_db(); student = current_student(db)
    if not student:
        db.close(); flash("Student account is not linked correctly. Contact the administrator.", "error"); return redirect(url_for("login"))
    rows = db.execute("""SELECT sub.code,sub.name subject_name,COUNT(a.id) total,
        COALESCE(SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END),0) present,
        COALESCE(SUM(CASE WHEN a.status='Absent' THEN 1 ELSE 0 END),0) absent,
        CASE WHEN COUNT(a.id)=0 THEN 0 ELSE ROUND(100.0*SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END)/COUNT(a.id),1) END pct
    ...
    styles = getSampleStyleSheet()
    title = ParagraphStyle("MyReportTitle", parent=styles["Title"], alignment=TA_CENTER, fontSize=18, leading=21, textColor=colors.HexColor("#17466d"), spaceAfter=2)
    sub = ParagraphStyle("MyReportSub", parent=styles["Normal"], alignment=TA_CENTER, fontSize=9, textColor=colors.HexColor("#6b8193"), spaceAfter=5)
    meta = ParagraphStyle("MyReportMeta", parent=styles["Normal"], fontSize=9, leading=13)
    story=[]
    logo_path = BASE_DIR / "static" / LOGO_FILE
    logo = Image(str(logo_path), width=20*mm, height=20*mm) if logo_path.exists() else ""
    header_text=[Paragraph(COLLEGE_NAME,title),Paragraph("STUDENT ATTENDANCE REPORT",sub),Paragraph(f"<b>Name:</b> {student['name']} &nbsp;&nbsp; <b>PRN:</b> {student['prn'] or '—'}",meta),Paragraph(f"<b>Course:</b> {student['course'] or 'B.Pharm'} &nbsp;&nbsp; <b>Year:</b> {student['year'] or '—'}",meta)]
    h=Table([[logo,header_text]],colWidths=[25*mm,150*mm]); h.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"MIDDLE"),("LEFTPADDING",(0,0),(-1,-1),0),("RIGHTPADDING",(0,0),(-1,-1),4),("BOX",(0,0),(-1,-1),0.5,colors.HexColor("#d7e6f2")),("BACKGROUND",(0,0),(-1,-1),colors.HexColor("#f5faff"))])); story += [h,Spacer(1,5*mm)]
    summary=Table([["TOTAL CLASSES","PRESENT","ABSENT","ATTENDANCE"], [str(total),str(present),str(absent),f"{overall}%"]], colWidths=[42*mm]*4)
    summary.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#eaf5ff")),("TEXTCOLOR",(0,0),(-1,0),colors.HexColor("#58728a")),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTNAME",(0,1),(-1,1),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),9),("ALIGN",(0,0),(-1,-1),"CENTER"),("BOX",(0,0),(-1,-1),0.5,colors.HexColor("#d9e6ef")),("INNERGRID",(0,0),(-1,-1),0.4,colors.HexColor("#d9e6ef")),("TOPPADDING",(0,0),(-1,-1),7),("BOTTOMPADDING",(0,0),(-1,-1),7)])); story += [summary,Spacer(1,6*mm),Paragraph("Subject-wise Attendance",styles["Heading3"])]
    data=[["#","Subject","Total","Present","Absent","Attendance %"]]+[[i+1,f"{r['code']} — {r['subject_name']}",r['total'],r['present'],r['absent'],f"{r['pct']}%"] for i,r in enumerate(rows)]
    table=Table(data,repeatRows=1,colWidths=[10*mm,88*mm,20*mm,20*mm,20*mm,25*mm]); table.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#dfeef9")),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),8.5),("GRID",(0,0),(-1,-1),0.4,colors.HexColor("#b8cbd8")),("ALIGN",(0,0),(0,-1),"CENTER"),("ALIGN",(2,1),(-1,-1),"CENTER"),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("ROWBACKGROUNDS",(0,1),(-1,-1),[colors.white,colors.HexColor("#f8fbfd")]),("TOPPADDING",(0,0),(-1,-1),6),("BOTTOMPADDING",(0,0),(-1,-1),6)])); story.append(table)
    story += [Spacer(1,5*mm),Paragraph("Generated from Y.N.P. College of Pharmacy Attendance Management System.",sub)]
    doc.build(story); pdf=buffer.getvalue(); buffer.close()
    return Response(pdf,mimetype="application/pdf",headers={"Content-Disposition":"inline; filename=my_attendance_report.pdf"})
