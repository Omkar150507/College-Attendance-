...
def student_my_report():
    ...
    if not student:
        ...
        flash("Student account is not linked correctly. Contact the administrator.", "error")
        return redirect(url_for("login"))
    rows = db.execute("""SELECT sub.code, sub.name subject_name,
        COUNT(a.id) total,
        COALESCE(SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END),0) present,
        COALESCE(SUM(CASE WHEN a.status='Absent' THEN 1 ELSE 0 END),0) absent,
        CASE WHEN COUNT(a.id)=0 THEN 0 ELSE ROUND(100.0*SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END)/COUNT(a.id),1) END pct
        FROM subjects sub LEFT JOIN attendance a ON a.subject_id=sub.id AND a.student_id=?
        WHERE sub.department_id=? AND sub.year=?
        GROUP BY sub.id ORDER BY sub.code""", (student["id"], student["department_id"], student["year"])).fetchall()
    total = sum(r["total"] for r in rows)
    present = sum(r["present"] for r in rows)
    absent = sum(r["absent"] for r in rows)
    overall = round((present / total * 100), 1) if total else 0
    db.close()
    return render_template("student_report.html", student=student, rows=rows, total=total, present=present, absent=absent, overall=overall)
...
def student_my_report_pdf():
    ...
    if not student:
        db.close()
        return Response("Student account is not linked correctly.", status=403, mimetype="text/plain")
    rows = db.execute("""SELECT sub.code, sub.name subject_name,
        COUNT(a.id) total,
        COALESCE(SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END),0) present,
        COALESCE(SUM(CASE WHEN a.status='Absent' THEN 1 ELSE 0 END),0) absent,
    ...
    try:
        ...
        styles = getSampleStyleSheet()
        title_style = ParagraphStyle("ReportTitle", parent=styles["Title"], alignment=TA_CENTER, fontSize=17, leading=21, textColor=colors.HexColor("#17324d"), spaceAfter=3)
        sub_style = ParagraphStyle("ReportSub", parent=styles["Normal"], alignment=TA_CENTER, fontSize=8.5, textColor=colors.HexColor("#667b8d"), leading=11)
        body_style = ParagraphStyle("Body", parent=styles["Normal"], fontSize=9, leading=12, textColor=colors.HexColor("#17324d"))
        story=[]
        story += [Paragraph("Y.N.P. College of Pharmacy", title_style), Paragraph("Student Attendance Management System", sub_style), Spacer(1,5*mm)]
        info = [["Student Name", student["name"] or "—", "PRN", student["prn"] or "—"], ["Course", student["course"] or "B.Pharm", "Year", student["year"] or "—"]]
        t=Table(info,colWidths=[28*mm,67*mm,20*mm,61*mm]); t.setStyle(TableStyle([("BACKGROUND",(0,0),(0,-1),colors.HexColor("#eef6fd")),("BACKGROUND",(2,0),(2,-1),colors.HexColor("#eef6fd")),("FONTNAME",(0,0),(-1,-1),"Helvetica"),("FONTNAME",(0,0),(0,-1),"Helvetica-Bold"),("FONTNAME",(2,0),(2,-1),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),8.5),("TEXTCOLOR",(0,0),(-1,-1),colors.HexColor("#17324d")),("BOX",(0,0),(-1,-1),0.6,colors.HexColor("#d8e5ef")),("INNERGRID",(0,0),(-1,-1),0.3,colors.HexColor("#e5edf4")),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("TOPPADDING",(0,0),(-1,-1),7),("BOTTOMPADDING",(0,0),(-1,-1),7)])); story += [t,Spacer(1,5*mm)]
        summary=[["Total Classes",str(total),"Present",str(present),"Absent",str(absent),"Attendance",f"{overall}%"]]
        s=Table(summary,colWidths=[25*mm,18*mm,22*mm,18*mm,18*mm,18*mm,24*mm,22*mm]); s.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#f3f8fc")),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,0),8),("TEXTCOLOR",(0,0),(-1,0),colors.HexColor("#17324d")),("BOX",(0,0),(-1,-1),0.6,colors.HexColor("#d8e5ef")),("INNERGRID",(0,0),(-1,-1),0.3,colors.HexColor("#e5edf4")),("ALIGN",(1,0),(1,0),"CENTER"),("ALIGN",(3,0),(3,0),"CENTER"),("ALIGN",(5,0),(5,0),"CENTER"),("ALIGN",(7,0),(7,0),"CENTER"),("TOPPADDING",(0,0),(-1,-1),7),("BOTTOMPADDING",(0,0),(-1,-1),7)])); story += [s,Spacer(1,6*mm),Paragraph("Subject-wise Attendance", ParagraphStyle("h",parent=styles["Heading2"],fontSize=11,textColor=colors.HexColor("#17324d"))),Spacer(1,2*mm)]
        data=[["#","Subject","Total","Present","Absent","Attendance %"]]
        for i,r in enumerate(rows,1): data.append([str(i),f"{r['code']} — {r['subject_name']}",str(r['total']),str(r['present']),str(r['absent']),f"{r['pct']}%"])
        if len(data)==1: data.append(["","No attendance records available.","","","",""])
        tab=Table(data,colWidths=[10*mm,82*mm,20*mm,22*mm,20*mm,28*mm],repeatRows=1)
        tab.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#0b4b82")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),8),("GRID",(0,0),(-1,-1),0.35,colors.HexColor("#dbe6ef")),("ROWBACKGROUNDS",(0,1),(-1,-1),[colors.white,colors.HexColor("#f7fafc")]),("ALIGN",(0,0),(0,-1),"CENTER"),("ALIGN",(2,1),(-1,-1),"CENTER"),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("TOPPADDING",(0,0),(-1,-1),6),("BOTTOMPADDING",(0,0),(-1,-1),6)])); story.append(tab); story += [Spacer(1,5*mm),Paragraph(f"Generated on {datetime.now().strftime('%d-%m-%Y %I:%M %p')}",ParagraphStyle("foot",parent=styles["Normal"],fontSize=7.5,textColor=colors.HexColor("#7a8e9f")))]
        doc.build(story); out.seek(0)
        safe_prn=(student["prn"] or "student").replace("/","-")
        return Response(out.getvalue(),mimetype="application/pdf",headers={"Content-Disposition":f"inline; filename=attendance_report_{safe_prn}.pdf"})
