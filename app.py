...
def take_attendance():
    ...
    if selected_subject:
        if session_type == "Practical":
            students = db.execute("SELECT * FROM students WHERE year=? AND batch=? ORDER BY prn, name", (selected_year, selected_batch)).fetchall() if selected_batch else []
        else:
            students = db.execute("SELECT * FROM students WHERE year=? ORDER BY prn, name", (selected_year,)).fetchall()
    else: students=[]
    # Keep the same numeric PRN order as Student Management on both SQLite and PostgreSQL.
