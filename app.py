...
def attendance_history():
    ...
    rows=db.execute(f"""SELECT a.attendance_date,a.lecture_no,a.lecture_time,
        COALESCE(a.session_type,'Lecture') session_type,COALESCE(a.batch,'') batch,
        a.scheduled_teacher_id,a.conducted_by_teacher_id,a.teacher_status,
        COALESCE(st.full_name,st.username,'') scheduled_teacher_name,
        COALESCE(ct.full_name,ct.username,'') conducted_teacher_name,
        sub.id subject_id,sub.code,sub.name subject_name,s.year,
        COUNT(a.id) total,
        SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END) present,
        SUM(CASE WHEN a.status='Absent' THEN 1 ELSE 0 END) absent
        FROM attendance a
        ...
        LEFT JOIN users st ON st.id=a.scheduled_teacher_id
        LEFT JOIN users ct ON ct.id=a.conducted_by_teacher_id
        WHERE {condition}
        GROUP BY a.attendance_date,a.lecture_no,a.lecture_time,a.session_type,a.batch,
                 a.scheduled_teacher_id,a.conducted_by_teacher_id,a.teacher_status,
                 st.full_name,st.username,ct.full_name,ct.username,
                 sub.id,sub.code,sub.name,s.year
        ORDER BY a.attendance_date DESC,a.lecture_no DESC,sub.code,s.year""",params).fetchall()
    subjects=teacher_or_admin_subjects(db)
    # History subject filter is intentionally all assigned subjects; the selected
    # year filter controls the session list itself.
    db.close()
    return render_template("attendance_history.html",rows=rows,subjects=subjects,years=YEAR_OPTIONS,
        year=year,subject_id=subject_id,from_date=from_date,to_date=to_date,
        session_type=session_type,batch=batch,batch_options=["Batch A","Batch B","Batch C","Batch D"])
...
def delete_attendance_session():
    ...
    lecture_no=request.form.get("lecture_no",type=int)
    ...
    if not attendance_date or not subject_id or not lecture_no:
        flash("Invalid attendance session.","error")
    ...
    sql="""DELETE FROM attendance
           WHERE subject_id=? AND attendance_date=? AND lecture_no=?
             AND COALESCE(session_type,'Lecture')=?"""
    params=[subject_id,attendance_date,lecture_no,session_type]
    if session_type=="Practical":
        sql += " AND COALESCE(batch,'')=?"
        params.append(batch)
    if lecture_time:
        sql += " AND COALESCE(lecture_time,'')=?"
        params.append(lecture_time)
    cur=db.execute(sql,tuple(params))
    deleted=cur.rowcount
    db.commit()
    log_activity("Attendance session deleted",f"{attendance_date} | {subject['code'] if subject else subject_id} | {session_type} {batch} | Lecture {lecture_no} | {deleted} records")
    db.close()
    flash(f"Attendance session deleted successfully ({deleted} student mark(s)).","success")
    return redirect(url_for("attendance_history"))
...
@staff_required
def qr_attendance():
    """Create a simple QR attendance session.

    Flow: Year -> Session -> Batch (Practical only) -> Subject -> Date ->
    Start/End time -> Teacher status -> Create QR.
    The timetable is used only to find the scheduled teacher when a matching
    day/subject/session/batch/time exists; selecting a timetable slot is not
    required to create a QR.
    """
    db=get_db()
    try:
        years=YEAR_OPTIONS[:]
        ...
        selected_year=(request.args.get('year') or request.form.get('year') or '').strip()
        ...
        attendance_date=(request.args.get('attendance_date') or request.form.get('attendance_date') or date.today().isoformat()).strip()
        try:
        ...
        allowed_statuses=(
            ...
            'Scheduled teacher absent',
            'Another teacher conducted the session',
        )
        if teacher_status not in allowed_statuses:
        ...
        # Only subjects belonging to the selected year are offered.
        subjects=[x for x in subjects_all if x['year']==selected_year] if selected_year else []
        if subject_id and not any(int(x['id'])==int(subject_id) for x in subjects):
            subject_id=None

        day_name=date.fromisoformat(attendance_date).strftime('%A')
        candidates=[]
        scheduled_teacher=None
        if selected_year and subject_id:
            candidates=timetable_candidates(db,selected_year,session_type,batch,subject_id,day_name)
            # Prefer a timetable entry whose time exactly matches the QR time.
            if start_time and end_time:
                for c in candidates:
                    if c['start_time']==start_time and c['end_time']==end_time:
                        scheduled_teacher=c
                        break
            if scheduled_teacher is None and len(candidates)==1:
                scheduled_teacher=candidates[0]

        if request.method=='POST':
            valid_subject=selected_year in YEAR_OPTIONS and subject_id and any(int(x['id'])==int(subject_id) for x in subjects)
            if not valid_subject:
                flash('Please select a valid subject for the selected year.','error')
            elif not start_time or not end_time or start_time>=end_time:
                flash('Please enter a valid Start Time and End Time.','error')
            elif session_type=='Practical' and batch not in batches:
                flash('Please select a practical batch.','error')
            elif teacher_status=='Another teacher conducted the session' and not conducted_by:
                flash('Please select the teacher who conducted the session.','error')
            else:
                # If the scheduled teacher is absent, the logged-in staff user is
                # treated as the substitute/conducting teacher unless another
                # teacher was explicitly selected.
                if teacher_status=='Scheduled teacher absent' and not conducted_by:
                    conducted_by=session['user_id']
                if conducted_by and not db.execute("SELECT id FROM users WHERE id=? AND role='teacher' AND approved=1",(conducted_by,)).fetchone():
                    flash('Selected conducting teacher is not valid.','error')
                else:
                    scheduled_id=scheduled_teacher['teacher_id'] if scheduled_teacher else None
                    if teacher_status=='Scheduled teacher present':
                        conducted_id=scheduled_id or session['user_id']
                    else:
                        conducted_id=conducted_by or session['user_id']
                    ...
                    db.execute("""INSERT INTO qr_sessions(
                        ...
                        scheduled_teacher_id,conducted_by_teacher_id,teacher_status,
                        expires_at,active)
                        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)""",
                        (token,subject_id,selected_year,attendance_date,1,
                         f'{start_time}-{end_time}',start_time,end_time,session_type,
                         batch if session_type=='Practical' else '',session['user_id'],
                         scheduled_id,conducted_id,teacher_status,exp.isoformat()))
                    db.commit()
                    db.close()
                    return redirect(url_for('qr_attendance',token=token))

        token=request.args.get('token','')
        qr=None; qr_session=None
        if token:
        ...
        return render_template('qr_attendance.html',
            subjects=subjects,years=years,today=date.today().isoformat(),
            attendance_date=attendance_date,qr=qr,qr_session=qr_session,
            token=token,batch=batch,session_type=session_type,teachers=teachers,
            selected_year=selected_year,selected_subject=subject_id,
            day_name=day_name,scheduled_teacher=scheduled_teacher,
            candidates=candidates,batch_options=batches,
            teacher_status=teacher_status,conducted_by=conducted_by,
