...
def student_my_report():
    ...
    if not student:
        ...
        flash("Student account is not linked correctly. Contact the administrator.", "error")
        return redirect(url_for("login"))
    rows = db.execute("""SELECT sub.code, sub.name AS subject_name,
        COUNT(a.id) total,
        COALESCE(SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END),0) present,
        COALESCE(SUM(CASE WHEN a.status='Absent' THEN 1 ELSE 0 END),0) absent,
        CASE WHEN COUNT(a.id)=0 THEN 0 ELSE ROUND(100.0*SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END)/COUNT(a.id),1) END pct
...
def student_my_report_pdf():
    ...
    if not student:
        db.close()
        return Response("Student account is not linked correctly.", status=401, mimetype="text/plain")
    rows = db.execute("""SELECT sub.code, sub.name AS subject_name,
        COUNT(a.id) total,
        COALESCE(SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END),0) present,
        COALESCE(SUM(CASE WHEN a.status='Absent' THEN 1 ELSE 0 END),0) absent,
    ...
    styles = getSampleStyleSheet()
    title = ParagraphStyle("StudentReportTitle", parent=styles["Title"], fontSize=19, leading=22, alignment=TA_LEFT, textColor=colors.HexColor("#073b70"), spaceAfter=2)
    subtitle = ParagraphStyle("StudentReportSub", parent=styles["Normal"], fontSize=9, leading=12, textColor=colors.HexColor("#61778b"))
    meta = ParagraphStyle("StudentReportMeta", parent=styles["Normal"], fontSize=9.5, leading=13, textColor=colors.HexColor("#29445d"))
    small = ParagraphStyle("StudentReportSmall", parent=styles["Normal"], fontSize=7.5, leading=10, textColor=colors.HexColor("#6b7e90"))

    story=[]
    logo_path = BASE_DIR / "static" / LOGO_FILE
    logo = Image(str(logo_path), width=21*mm, height=21*mm) if logo_path.exists() else ""
    header_text=[Paragraph(COLLEGE_NAME, title),
                 Paragraph("Student Attendance Report", subtitle),
                 Spacer(1, 2*mm),
                 Paragraph(f"<b>Student:</b> {student['name']} &nbsp;&nbsp; <b>PRN:</b> {student['prn'] or '—'}", meta),
                 Paragraph(f"<b>Year:</b> {student['year'] or '—'} &nbsp;&nbsp; <b>Course:</b> {student['course'] or 'B.Pharm'}", meta)]
    head=Table([[logo, header_text]], colWidths=[25*mm, 155*mm])
    head.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"MIDDLE"),("LEFTPADDING",(0,0),(-1,-1),0),("RIGHTPADDING",(0,0),(-1,-1),5),("TOPPADDING",(0,0),(-1,-1),0),("BOTTOMPADDING",(0,0),(-1,-1),0)]))
    story += [head, Spacer(1,5*mm)]

    summary_data=[["Total Classes", "Present", "Absent", "Overall Attendance"],
                  [str(total), str(present), str(absent), f"{overall}%"]]
    summary_table=Table(summary_data, colWidths=[42*mm]*4)
    summary_table.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#eaf4fc")),
                                       ("TEXTCOLOR",(0,0),(-1,0),colors.HexColor("#52708a")),
                                       ("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),
                                       ("FONTNAME",(0,1),(-1,1),"Helvetica-Bold"),
                                       ("FONTSIZE",(0,0),(-1,-1),8.5),
                                       ("ALIGN",(0,0),(-1,-1),"CENTER"),
                                       ("VALIGN",(0,0),(-1,-1),"MIDDLE"),
                                       ("BOX",(0,0),(-1,-1),0.5,colors.HexColor("#d8e5ef")),
                                       ("INNERGRID",(0,0),(-1,-1),0.4,colors.HexColor("#d8e5ef")),
                                       ("TOPPADDING",(0,0),(-1,-1),7),("BOTTOMPADDING",(0,0),(-1,-1),7)]))
    story += [summary_table, Spacer(1,6*mm)]

    data=[["#","Subject Code","Subject Name","Total","Present","Absent","Attendance %"]]
    for i,r in enumerate(rows,1):
        data.append([str(i),r["code"],r["subject_name"],str(r["total"]),str(r["present"]),str(r["absent"]),f"{r['pct']}%"])
    if not rows:
        data.append(["—","—","No attendance records available","0","0","0","0%"])
    table=Table(data, repeatRows=1, colWidths=[9*mm,27*mm,69*mm,18*mm,20*mm,18*mm,28*mm])
    table.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#0b4b82")),
                               ...
                               ("VALIGN",(0,0),(-1,-1),"MIDDLE"),
                               ("GRID",(0,0),(-1,-1),0.4,colors.HexColor("#cbd9e5")),
                               ("ROWBACKGROUNDS",(0,1),(-1,-1),[colors.white,colors.HexColor("#f6f9fc")]),
                               ("TOPPADDING",(0,0),(-1,-1),6),("BOTTOMPADDING",(0,0),(-1,-1),6)]))
    story += [table, Spacer(1,5*mm), Paragraph("Generated by Y.N.P. College of Pharmacy Attendance Management System.", small)]
    doc.build(story)
    pdf=buffer.getvalue(); buffer.close()
    return Response(pdf, mimetype="application/pdf",
                    headers={"Content-Disposition":"attachment; filename=student_attendance_report.pdf"})
