...
def student_dashboard():
    ...
    try:
        ...
        subjects = db.execute("SELECT id,code,name FROM subjects WHERE year=? ORDER BY code", (student["year"],)).fetchall()
        today_rows = db.execute("""SELECT sub.code,sub.name subject_name,a.status,
            ...
            WHERE sub.year=? ORDER BY sub.code,a.lecture_no""",
            (student["id"],today,student["year"])).fetchall()
        history_sql="""SELECT a.attendance_date,sub.code,sub.name subject_name,a.status,
            COALESCE(a.lecture_no,1) lecture_no,COALESCE(a.lecture_time,'') lecture_time
            FROM attendance a JOIN subjects sub ON sub.id=a.subject_id WHERE a.student_id=?"""
        hp=[student["id"]]
        if selected_date:
            history_sql += " AND a.attendance_date=?"; hp.append(selected_date)
        if selected_subject:
            try: selected_subject_id=int(selected_subject)
            except ValueError: selected_subject_id=0
            if selected_subject_id:
                history_sql += " AND a.subject_id=?"; hp.append(selected_subject_id)
        history_sql += " ORDER BY a.attendance_date DESC,sub.code,a.lecture_no"
        history=db.execute(history_sql,tuple(hp)).fetchall()
        summary=db.execute("""SELECT sub.code,sub.name subject_name,COUNT(a.id) total,
            ...
            WHERE sub.year=? GROUP BY sub.id ORDER BY sub.code""",(student["id"],student["year"])).fetchall()
        ...
        overall=round(100.0*present/total,1) if total else 0
        return render_template("student_dashboard.html",student=student,today_rows=today_rows,summary=summary,overall=overall,history=history,subjects=subjects,selected_date=selected_date,selected_subject=selected_subject,today=today)
    except Exception:
        app.logger.exception("Student dashboard error")
        db.close()
        flash("Student dashboard could not be loaded. Please contact the administrator.","error")
        session.clear()
        return redirect(url_for("login"))
    finally:
        try: db.close()
...
def qr_attendance():
    ...
    try:
        ...
        selected_year=(request.values.get('year') or '1st Year').strip()
        if selected_year not in years: selected_year='1st Year'
        session_type=(request.values.get('session_type') or 'Lecture').strip()
        if session_type not in ('Lecture','Practical'): session_type='Lecture'
        batch=(request.values.get('batch') or '').strip() if session_type=='Practical' else ''
        if batch not in ('','Batch A','Batch B','Batch C','Batch D'): batch=''
        subject_id=request.values.get('subject_id',type=int)
        attendance_date=request.values.get('attendance_date') or date.today().isoformat()
        try: date.fromisoformat(attendance_date)
        except ValueError: attendance_date=date.today().isoformat()
        start_time=(request.values.get('start_time') or '').strip()
        end_time=(request.values.get('end_time') or '').strip()
        teacher_status=(request.values.get('teacher_status') or 'Scheduled teacher present').strip()
        allowed_status=('Scheduled teacher present','Scheduled teacher absent','Another teacher conducted the session')
        if teacher_status not in allowed_status: teacher_status='Scheduled teacher present'
        conducted_by=request.values.get('conducted_by',type=int)
        subjects=[x for x in subjects_all if x['year']==selected_year]
        if subject_id and not any(int(x['id'])==int(subject_id) for x in subjects): subject_id=None
        teachers=all_teacher_rows(db)
        day_name=date.fromisoformat(attendance_date).strftime('%A')
        # Automatically find the scheduled teacher from timetable; no timetable slot selection is required on the QR page.
        scheduled=None
        if subject_id:
            tr=db.execute("""SELECT teacher_id,start_time,end_time FROM timetable
                WHERE year=? AND day_of_week=? AND subject_id=?
                AND COALESCE(session_type,'Lecture')=?
                AND (COALESCE(batch,'')=? OR ?='Lecture')
                ORDER BY start_time LIMIT 1""",
                (selected_year,day_name,subject_id,session_type,batch,session_type)).fetchone()
            scheduled=tr['teacher_id'] if tr else None
            if not start_time and tr: start_time=tr['start_time'] or ''
            if not end_time and tr: end_time=tr['end_time'] or ''
        if request.method=='POST':
            if not subject_id:
                flash('Please select a subject for the selected year.','error')
            elif session_type=='Practical' and not batch:
                flash('Please select the practical batch.','error')
            elif not start_time or not end_time or start_time>=end_time:
                flash('Please enter a valid Time From and Time To.','error')
            elif teacher_status in ('Scheduled teacher absent','Another teacher conducted the session') and not conducted_by:
                flash('Please select the teacher who conducted the session.','error')
            else:
                ...
                db.execute("""INSERT INTO qr_sessions(token,subject_id,year,attendance_date,lecture_no,lecture_time,start_time,end_time,session_type,batch,teacher_id,scheduled_teacher_id,conducted_by_teacher_id,teacher_status,expires_at,active)
                    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)""",
                    (token,subject_id,selected_year,attendance_date,1,f'{start_time}-{end_time}',start_time,end_time,session_type,batch,session['user_id'],scheduled,conducted_by,teacher_status,exp.isoformat()))
                db.commit()
        ...
        if token:
            qr_session=db.execute("""SELECT q.*,s.code,s.name subject_name,
                ...
                LEFT JOIN users st ON st.id=q.scheduled_teacher_id LEFT JOIN users ct ON ct.id=q.conducted_by_teacher_id
                WHERE q.token=? AND q.active=1""",(token,)).fetchone()
            if qr_session:
                try:
                    if datetime.fromisoformat(qr_session['expires_at'])<datetime.utcnow(): qr_session=None
                except Exception: qr_session=None
        ...
        return render_template('qr_attendance.html',years=years,subjects=subjects,selected_year=selected_year,session_type=session_type,batch=batch,selected_subject=subject_id,attendance_date=attendance_date,start_time=start_time,end_time=end_time,teacher_status=teacher_status,conducted_by=conducted_by,teachers=teachers,qr=qr,qr_session=qr_session,token=token,scheduled_teacher_id=scheduled,day_name=day_name)
    except Exception:
        db.rollback(); app.logger.exception('QR attendance page error'); flash('QR Attendance could not be loaded. Please check the selected year and subject.','error'); return redirect(url_for('qr_attendance'))
    finally:
        try: db.close()
        except Exception: pass
