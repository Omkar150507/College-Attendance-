...
def init_db():
    ...
    db.execute("CREATE INDEX IF NOT EXISTS idx_timetable_week ON timetable(year,day_of_week,start_time)")
    ...
    # Department/HOD functionality has been removed. Convert any legacy HOD account to a regular teacher.
    db.execute("UPDATE users SET role='teacher' WHERE role='hod'")
    ...
    if request.method == "POST":
        ...
        if user and check_password_hash(user["password"], password):
            ...
            if user["role"] == "student":
                return redirect(url_for("student_dashboard"))
            if user["role"] == "hod":
                # Legacy HOD accounts are treated as regular teachers now that
                # Department/HOD functionality has been removed from the system.
...
def _time_minutes(value):
    try:
        h, m = [int(x) for x in str(value or '').split(':', 1)]
        return h * 60 + m
    except Exception:
        return 0
...
def timetable_scope_sql(role, user_id, student):
    where=[]; params=[]
    if role=='teacher':
        where.append('t.teacher_id=?'); params.append(user_id)
    elif role=='student':
        where += ['t.year=?', "(COALESCE(t.session_type,'Lecture')='Lecture' OR COALESCE(t.batch,'')=?)"]
        params += [student['year'], student['batch'] or '']
    return where, params


def _timetable_scope(role, user_id, student, year, session_type, batch):
    where=[]; params=[]
    if role == 'teacher':
        where.append('t.teacher_id=?'); params.append(user_id)
    elif role == 'student':
        if not student:
            ...
            where += ['t.year=?', "(COALESCE(t.session_type,'Lecture')='Lecture' OR COALESCE(t.batch,'')=?)"]
            params += [student['year'], student['batch'] or '']
    if role in ('admin','teacher') and year:
        where.append('t.year=?'); params.append(year)
    if session_type in ('Lecture','Practical'):
        where.append("COALESCE(t.session_type,'Lecture')=?"); params.append(session_type)
    if session_type == 'Practical' and batch in ('Batch A','Batch B','Batch C','Batch D'):
        where.append("COALESCE(t.batch,'')=?"); params.append(batch)
    return where, params

def _load_timetable_rows(db, role, user_id, student, year='', day='', view='week', session_type='', batch=''):
    where,params=_timetable_scope(role,user_id,student,year,session_type,batch)
    if view == 'day':
        where.append('t.day_of_week=?'); params.append(day)
    condition=' AND '.join(where) if where else '1=1'
    return db.execute(f"""SELECT t.*, COALESCE(t.session_type,'Lecture') session_type,
        ...
        WHERE {condition}
        ORDER BY CASE t.day_of_week WHEN 'Monday' THEN 1 WHEN 'Tuesday' THEN 2 WHEN 'Wednesday' THEN 3 WHEN 'Thursday' THEN 4 WHEN 'Friday' THEN 5 WHEN 'Saturday' THEN 6 ELSE 7 END,
                 t.start_time,t.end_time,t.year,t.id""", tuple(params)).fetchall()

def _timetable_slot_rows(rows, days):
    slots=[]
    seen=set()
    for r in rows:
        key=(r['start_time'],r['end_time'])
        if key not in seen:
            seen.add(key); slots.append(key)
    slots.sort(key=lambda x: (_time_minutes(x[0]), _time_minutes(x[1])))
    return slots

def _timetable_display_grid(rows, days):
    """Build a compact grid where a long practical spans the lecture slots it covers.
    Lecture slots remain the normal rows; a 09:30-12:30 practical is rendered once
    with a four-row span instead of creating an extra 09:30-12:30 row.
    """
    lecture_slots=[]; seen=set()
    for r in rows:
        if r['session_type'] == 'Practical':
            continue
        key=(r['start_time'],r['end_time'])
        if key not in seen:
            seen.add(key); lecture_slots.append(key)
    lecture_slots.sort(key=lambda x: (_time_minutes(x[0]), _time_minutes(x[1])))

    # Keep a practical as its own row only when it is not covered by the normal
    # lecture slots. A 09:30-12:30 practical is therefore absorbed into the
    # four 45-minute rows, while a practical at another time remains one row.
    for r in rows:
        if r['session_type'] != 'Practical':
            continue
        pst,pen=_time_minutes(r['start_time']),_time_minutes(r['end_time'])
        covered=any(pst <= _time_minutes(st) and pen >= _time_minutes(en)
                    for st,en in lecture_slots)
        if not covered:
            key=(r['start_time'],r['end_time'])
            if key not in seen:
                seen.add(key); lecture_slots.append(key)
    ...
    for r in rows:
        if r['session_type'] == 'Practical':
            practicals_by_day.setdefault(r['day_of_week'], []).append(r)

    grid=[]
    for st,en in lecture_slots:
        row={'start_time':st,'end_time':en,'cells':{}}
        for d in days:
            lectures=[r for r in rows if r['day_of_week']==d and r['session_type']!='Practical' and r['start_time']==st and r['end_time']==en]
            spanning=[]
            skip=False
            for pr in practicals_by_day.get(d,[]):
                pst,pen=pr['start_time'],pr['end_time']
                if _time_minutes(pst) <= _time_minutes(st) and _time_minutes(pen) >= _time_minutes(en):
                    # This practical starts before/at this row. Render it only in its first row.
                    if _time_minutes(pst) == _time_minutes(st):
                        spanning.append(pr)
                    else:
                        skip=True
            if spanning:
                span=sum(1 for xst,xen in lecture_slots if _time_minutes(xst)>=_time_minutes(st) and _time_minutes(xen)<=_time_minutes(max([pr['end_time'] for pr in spanning], key=_time_minutes)))
                row['cells'][d]={'entries':lectures+spanning,'rowspan':max(1,span),'skip':False}
            elif skip:
                row['cells'][d]={'entries':[],'rowspan':0,'skip':True}
            else:
                row['cells'][d]={'entries':lectures,'rowspan':1,'skip':False}
        grid.append(row)
...
def timetable():
    ...
    if role not in ('admin','teacher','student'):
        db.close(); flash('Timetable is available only for Admin, Teacher and Student accounts.','error'); return redirect(url_for('dashboard'))
    ...
    if request.method=='POST':
        ...
        subject_row=db.execute('SELECT id,year FROM subjects WHERE id=?',(subject_id,)).fetchone() if subject_id else None
        if year not in YEAR_OPTIONS or day not in days or not start or not end or start>=end or not subject_id or not teacher_id or session_type not in ('Lecture','Practical'):
            flash('Please fill all timetable fields correctly. Start time must be before end time.','error')
        elif not subject_row or subject_row['year']!=year:
            flash('Please select a subject belonging to the selected year.','error')
        elif session_type=='Practical' and batch not in batches:
            flash('Select a valid practical batch.','error')
        else:
            # Department is intentionally not used by the timetable UI or logic.
            db.execute("INSERT INTO timetable(department_id,year,day_of_week,lecture_no,start_time,end_time,subject_id,teacher_id,room,session_type,batch) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                       (None,year,day,1,start,end,subject_id,teacher_id,room,session_type,batch if session_type=='Practical' else ''))
            db.commit(); log_activity('Timetable entry added',f'{year} | {day} | {start}-{end} | {subject_id} | {session_type} {batch}')
            flash('Timetable entry added successfully.','success')

    selected_year=request.args.get('year','').strip()
    selected_day=request.args.get('day','').strip()
    view=request.args.get('view','week').strip().lower()
    session_type=request.args.get('session_type','').strip()
    batch=request.args.get('batch','').strip()
    if view not in ('day','week'): view='week'
    if selected_day not in days: selected_day=date.today().strftime('%A')
    if role=='admin' and not selected_year: selected_year='1st Year'
    student=current_student(db) if role=='student' else None
    if role=='student' and student: selected_year=student['year']; batch=student['batch'] or ''
    rows=_load_timetable_rows(db,role,session.get('user_id'),student,selected_year,selected_day,view,session_type,batch)
    subjects=db.execute('SELECT id,code,name,year FROM subjects ORDER BY CASE year WHEN \'1st Year\' THEN 1 WHEN \'2nd Year\' THEN 2 WHEN \'3rd Year\' THEN 3 ELSE 4 END,code').fetchall()
    teachers=all_teacher_rows(db)
    slots=_timetable_slot_rows(rows,days)
    grid_rows=_timetable_display_grid(rows, [selected_day] if view=='day' else days)
    db.close()
    return render_template('timetable.html',rows=rows,subjects=subjects,teachers=teachers,years=YEAR_OPTIONS,selected_year=selected_year,selected_day=selected_day,days=days,batch_options=batches,view=view,session_type=session_type,batch=batch,slots=slots,grid_rows=grid_rows)
...
def export_timetable_pdf():
    ...
    view=request.args.get('view','week').strip().lower()
    if view not in ('day','week'): view='week'
    session_type=request.args.get('session_type','').strip(); batch=request.args.get('batch','').strip()
    if role=='admin' and not selected_year: selected_year='1st Year'
    student=current_student(db) if role=='student' else None
    if role=='student':
        if not student:
            db.close(); return Response('Student record not found.',status=404)
        selected_year=student['year']; batch=student['batch'] or ''
    if role not in ('admin','teacher','student'):
        db.close(); return Response('Not authorized.',status=403)
    rows=_load_timetable_rows(db,role,session.get('user_id'),student,selected_year,selected_day,view,session_type,batch)
    ayrow=db.execute("SELECT value FROM app_settings WHERE key='academic_year'").fetchone()
    academic_year=(ayrow['value'] if ayrow and ayrow['value'] else '2026-27')
    # Bottom details table: show lecture subjects only.
    # A practical must NOT create an extra row here. If the same subject/teacher
    # also has a practical, use its lab only as the lab value for the existing row.
    legend=[]; seen={};
    for r in rows:
        if r['session_type'] != 'Lecture':
            continue
        key=(r['subject_code'],r['subject_name'],r['teacher_name'])
        if key not in seen:
            item=[r['subject_code'],r['subject_name'],r['teacher_name'],r['room'] or '']
            seen[key]=item; legend.append(item)

    # Fill the lab from a matching practical without adding a practical row.
    for r in rows:
        if r['session_type'] != 'Practical' or not r['room']:
            continue
        key=(r['subject_code'],r['subject_name'],r['teacher_name'])
        if key in seen and not seen[key][3]:
            seen[key][3]=r['room']
    db.close()

    buffer=io.BytesIO()
    doc=SimpleDocTemplate(buffer,pagesize=landscape(A4),rightMargin=8*mm,leftMargin=8*mm,topMargin=7*mm,bottomMargin=11*mm)
    styles=getSampleStyleSheet()
    title=ParagraphStyle('TTH1',parent=styles['Title'],alignment=TA_CENTER,fontName='Helvetica-Bold',fontSize=19,leading=21,textColor=colors.HexColor('#123f73'),spaceAfter=0)
    institute=ParagraphStyle('TTInst',parent=styles['Normal'],alignment=TA_CENTER,fontName='Helvetica-Bold',fontSize=10,leading=12,textColor=colors.HexColor('#123f73'))
    small=ParagraphStyle('TTSmall',parent=styles['Normal'],alignment=TA_CENTER,fontSize=8.3,leading=10,textColor=colors.HexColor('#253f5a'))
    band=ParagraphStyle('TTBand',parent=styles['Normal'],alignment=TA_CENTER,fontName='Helvetica-Bold',fontSize=13,leading=15,textColor=colors.white)
    cell=ParagraphStyle('TTCell',parent=styles['Normal'],alignment=TA_CENTER,fontSize=7.1,leading=8.2,textColor=colors.HexColor('#172f49'))
    cell_left=ParagraphStyle('TTCellL',parent=cell,alignment=TA_LEFT)
    head=ParagraphStyle('TTHead',parent=cell,fontName='Helvetica-Bold',fontSize=8.1,textColor=colors.HexColor('#102f50'))
    note=ParagraphStyle('TTNote',parent=styles['Normal'],fontSize=7.6,leading=9,textColor=colors.HexColor('#263b52'))

    story=[]
    logo_path=BASE_DIR/'static'/LOGO_FILE
    logo=Image(str(logo_path),width=27*mm,height=27*mm) if logo_path.exists() else ''
    code_box=Table([[Paragraph('<b>DTE CODE: 3533</b>',small),Paragraph('<b>MSBTE CODE: 12311</b>',small),Paragraph('<b>PCI CODE: 6184</b>',small)]],colWidths=[42*mm,42*mm,42*mm])
    code_box.setStyle(TableStyle([('BOX',(0,0),(-1,-1),0.7,colors.HexColor('#174f8b')),('INNERGRID',(0,0),(-1,-1),0.45,colors.HexColor('#8fb0ca')),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('TOPPADDING',(0,0),(-1,-1),3),('BOTTOMPADDING',(0,0),(-1,-1),3),('LEFTPADDING',(0,0),(-1,-1),3),('RIGHTPADDING',(0,0),(-1,-1),3)]))
    header_lines=[Paragraph('V. M. P. PATH, KRUSHI PRATISHTHAN',institute),Paragraph(str(COLLEGE_NAME).upper(),title),Paragraph('Recognized by Govt. of Maharashtra',small),Paragraph('Affiliated by DBATU, Lonere, Raigad & Affiliated by MSBTE Mumbai',small),code_box,Paragraph('Off.: Asangaon, Station - Vangaon (Western Railway), Tal. Dahanu, Dist. Palghar - 401 103.',small),Paragraph('E-mail: ynpbpharm2022@gmail.com &nbsp;&nbsp; Website: www.ynpbpharm.in &nbsp;&nbsp; Mobile: 7249579956',small)]
    hdr=Table([[logo,header_lines]],colWidths=[31*mm,233*mm])
    hdr.setStyle(TableStyle([('VALIGN',(0,0),(-1,-1),'MIDDLE'),('ALIGN',(1,0),(1,0),'CENTER'),('LINEBELOW',(1,0),(1,0),1.2,colors.HexColor('#174f8b')),('LEFTPADDING',(0,0),(-1,-1),2),('RIGHTPADDING',(0,0),(-1,-1),2),('TOPPADDING',(0,0),(-1,-1),1),('BOTTOMPADDING',(0,0),(-1,-1),1)]))
    story += [hdr,Spacer(1,2*mm)]
    scope_year=_year_heading(selected_year or 'All Years')
    scope=f'{scope_year} B. PHARMACY  |  A.Y.: {academic_year}'
    if view=='day': scope += f'  |  {selected_day.upper()}'
    banner=Table([[Paragraph(scope,band)]],colWidths=[264*mm])
    banner.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),colors.HexColor('#07579a')),('BOX',(0,0),(-1,-1),0.6,colors.HexColor('#07579a')),('TOPPADDING',(0,0),(-1,-1),6),('BOTTOMPADDING',(0,0),(-1,-1),6)]))
    story += [banner,Spacer(1,1.8*mm)]

    if view=='week':
        grid_rows=_timetable_display_grid(rows, days)
        header_row=[Paragraph('DAY & TIME',head)]+[Paragraph(d[:4].upper() if d=='Thursday' else d[:3].upper(),head) for d in days]
        data=[header_row]
        span_specs=[]
        for ridx,g in enumerate(grid_rows, start=1):
            cells=[Paragraph(f'<b>{_format_time_12(g["start_time"])}</b> To <b>{_format_time_12(g["end_time"])}</b>',cell_left)]
            for cidx,d in enumerate(days, start=1):
                c=g['cells'][d]
                if c.get('skip'):
                ...
                bits=[]
                for r in c['entries']:
                    subj=r['subject_name'] or r['subject_code']
                    details=f'<b>{subj}</b>'
                    if r['session_type']=='Practical': details += f'<br/><b>{r["batch"] or "Batch"}</b>'
                    bits.append(details)
                cells.append(Paragraph('<br/><br/>'.join(bits) if bits else '—',cell))
                if c.get('rowspan',1)>1:
                    # Table column is shifted by one because column 0 is time.
                    span_specs.append(('SPAN',(cidx,ridx),(cidx,ridx+c['rowspan']-1)))
            data.append(cells)
        table=Table(data,repeatRows=1,colWidths=[48*mm]+[36*mm]*6)
    else:
        grid_rows=_timetable_display_grid(rows, [selected_day])
        data=[[Paragraph('TIME',head),Paragraph(selected_day[:3].upper(),head)]]
        span_specs=[]
        for ridx,g in enumerate(grid_rows, start=1):
            c=g['cells'][selected_day]
            row=[Paragraph(f'<b>{_format_time_12(g["start_time"])}</b> To <b>{_format_time_12(g["end_time"])}</b>',cell_left)]
            if c.get('skip'):
                row.append(Paragraph('',cell))
            else:
                bits=[]
                for r in c['entries']:
                    subj=r['subject_name'] or r['subject_code']; txt=f'<b>{subj}</b>'
                    if r['session_type']=='Practical': txt += f'<br/><b>{r["batch"] or "Batch"}</b>'
                    bits.append(txt)
                row.append(Paragraph('<br/><br/>'.join(bits) if bits else '—',cell))
                if c.get('rowspan',1)>1:
                    span_specs.append(('SPAN',(1,ridx),(1,ridx+c['rowspan']-1)))
            data.append(row)
        table=Table(data,repeatRows=1,colWidths=[58*mm,206*mm])
    table.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.HexColor('#d9ecfb')),('BOX',(0,0),(-1,-1),0.65,colors.HexColor('#7ea8c9')),('INNERGRID',(0,0),(-1,-1),0.35,colors.HexColor('#9ab9d2')),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('TOPPADDING',(0,0),(-1,-1),4.3),('BOTTOMPADDING',(0,0),(-1,-1),4.3),('LEFTPADDING',(0,0),(-1,-1),4),('RIGHTPADDING',(0,0),(-1,-1),4)] + span_specs))
    story += [table,Spacer(1,3.5*mm)]

    if legend:
        ldata=[[Paragraph('Subjects',head),Paragraph('Subject Teacher',head),Paragraph('Lab',head)]]
        for code,name,teacher,room in legend:
            ldata.append([Paragraph(f'<b>{name or code or "—"}</b>',cell_left),Paragraph(teacher or '—',cell_left),Paragraph(room or '—',cell_left)])
        lt=Table(ldata,repeatRows=1,colWidths=[108*mm,88*mm,68*mm])
        lt.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.HexColor('#d9ecfb')),('BOX',(0,0),(-1,-1),0.55,colors.HexColor('#7ea8c9')),('INNERGRID',(0,0),(-1,-1),0.35,colors.HexColor('#9ab9d2')),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('TOPPADDING',(0,0),(-1,-1),3.5),('BOTTOMPADDING',(0,0),(-1,-1),3.5),('LEFTPADDING',(0,0),(-1,-1),4),('RIGHTPADDING',(0,0),(-1,-1),4)]))
        story.append(lt)

    def page_frame(canvas,doc_obj):
        canvas.saveState(); w,h=landscape(A4)
        canvas.setStrokeColor(colors.HexColor('#1a4f87')); canvas.setLineWidth(1.2); canvas.rect(5*mm,5*mm,w-10*mm,h-10*mm,fill=0,stroke=1)
        canvas.setFillColor(colors.HexColor('#084b83')); canvas.setFont('Helvetica',7.5); canvas.drawString(9*mm,7*mm,'Y.N.P. College of Pharmacy  •  Academic Timetable'); canvas.drawRightString(w-9*mm,7*mm,f'Page {doc_obj.page}')
        canvas.restoreState()
    doc.build(story,onFirstPage=page_frame,onLaterPages=page_frame)
    pdf=buffer.getvalue(); buffer.close()
    fname=f"timetable_{(selected_year or 'all').replace(' ','_')}_{'week' if view=='week' else selected_day.lower()}.pdf"
...
@app.route("/student/my-report")
@student_required
def student_my_report():
    """Show the logged-in student's own attendance report."""
    db = get_db()
    student = current_student(db)
    if not student:
        db.close()
    ...
    summary = db.execute("""SELECT sub.code, sub.name subject_name,
        ...
        CASE WHEN COUNT(a.id)=0 THEN 0
             ELSE ROUND(100.0*SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END)/COUNT(a.id),1) END pct
        FROM subjects sub
    ...
    history = db.execute("""SELECT a.attendance_date, sub.code, sub.name subject_name,
        COALESCE(a.lecture_no,1) lecture_no, COALESCE(a.lecture_time,'') lecture_time, a.status
        FROM attendance a JOIN subjects sub ON sub.id=a.subject_id
        WHERE a.student_id=?
        ORDER BY a.attendance_date DESC, sub.code, a.lecture_no""",
        (student["id"],)).fetchall()
    ...
    overall = round((present / total * 100), 1) if total else 0
    db.close()
    return render_template("student_report.html", student=student, summary=summary, history=history,
                           total=total, present=present, absent=absent, overall=overall)


@app.route("/student/my-report.pdf")
@student_required
def student_my_report_pdf():
    """Export only the logged-in student's attendance report as PDF."""
    db = get_db()
    student = current_student(db)
    if not student:
        db.close()
    ...
    styles = getSampleStyleSheet()
    title = ParagraphStyle("StudentReportTitle", parent=styles["Title"], alignment=TA_CENTER,
                           fontSize=19, leading=22, textColor=colors.HexColor("#123f73"), spaceAfter=2)
    subtitle = ParagraphStyle("StudentReportSub", parent=styles["Normal"], alignment=TA_CENTER,
                              fontSize=9, leading=12, textColor=colors.HexColor("#475569"))
    section = ParagraphStyle("StudentReportSection", parent=styles["Heading2"], fontSize=12, leading=15,
                             textColor=colors.HexColor("#123f73"), spaceBefore=8, spaceAfter=5)
    body = ParagraphStyle("StudentReportBody", parent=styles["Normal"], fontSize=9, leading=12,
                          textColor=colors.HexColor("#263b52"))
    cell = ParagraphStyle("StudentReportCell", parent=styles["Normal"], fontSize=8, leading=10,
                          textColor=colors.HexColor("#172f49"))
    head = ParagraphStyle("StudentReportHead", parent=cell, fontName="Helvetica-Bold",
    ...
    header_lines = [
        Paragraph(xml_escape(str(COLLEGE_NAME)), title),
        Paragraph("Student Attendance Report", ParagraphStyle("RSubTitle", parent=title, fontSize=13, leading=16)),
        Paragraph("Generated from the student's own attendance record", subtitle),
    ]
    hdr = Table([[logo, header_lines]], colWidths=[25*mm, 150*mm])
    hdr.setStyle(TableStyle([
        ("VALIGN", (0,0), (-1,-1), "MIDDLE"), ("ALIGN", (1,0), (1,0), "CENTER"),
        ("LINEBELOW", (1,0), (1,0), 1, colors.HexColor("#174f8b")),
    ...
    info = [
        [Paragraph("Student Name", body), Paragraph(xml_escape(str(student["name"] or "—")), body),
         Paragraph("PRN", body), Paragraph(xml_escape(str(student["prn"] or "—")), body)],
        [Paragraph("Course", body), Paragraph(xml_escape(str(student["course"] or "B.Pharm")), body),
         Paragraph("Year", body), Paragraph(xml_escape(str(student["year"] or "—")), body)],
    ]
    info_table = Table(info, colWidths=[28*mm, 62*mm, 22*mm, 63*mm])
    info_table.setStyle(TableStyle([
        ("BACKGROUND", (0,0), (0,-1), colors.HexColor("#eef5fb")),
        ("BACKGROUND", (2,0), (2,-1), colors.HexColor("#eef5fb")),
    ...
    stats = Table([[
        Paragraph(f"<b>Total</b><br/>{total}", body),
        Paragraph(f"<b>Present</b><br/>{present}", body),
        Paragraph(f"<b>Absent</b><br/>{absent}", body),
        Paragraph(f"<b>Overall Attendance</b><br/>{overall}%", body),
    ...
    story += [stats, Spacer(1, 4*mm), Paragraph("Subject-wise Attendance", section)]

    subject_data = [[Paragraph("Subject", head), Paragraph("Total", head), Paragraph("Present", head),
                     Paragraph("Absent", head), Paragraph("Attendance %", head)]]
    for r in summary:
        subject_data.append([
            Paragraph(xml_escape(f"{r['code']} - {r['subject_name']}"), cell),
            Paragraph(str(r["total"]), cell), Paragraph(str(r["present"]), cell),
            Paragraph(str(r["absent"]), cell), Paragraph(f"{r['pct']}%", cell)
        ])
    if len(subject_data) == 1:
        subject_data.append([Paragraph("No subject attendance found.", cell), "", "", "", ""])
    subject_table = Table(subject_data, colWidths=[92*mm, 20*mm, 20*mm, 20*mm, 23*mm], repeatRows=1)
    ...
    if history:
        story += [Spacer(1, 4*mm), Paragraph("Attendance History", section)]
        hist_data = [[Paragraph("Date", head), Paragraph("Subject", head), Paragraph("Lecture", head),
                      Paragraph("Time", head), Paragraph("Status", head)]]
        for r in history:
            hist_data.append([Paragraph(str(r["attendance_date"] or "—"), cell),
                              Paragraph(xml_escape(f"{r['code']} - {r['subject_name']}"), cell),
                              Paragraph(f"L{r['lecture_no']}", cell),
                              Paragraph(xml_escape(str(r["lecture_time"] or "—")), cell),
                              Paragraph(xml_escape(str(r["status"] or "—")), cell)])
        hist_table = Table(hist_data, colWidths=[28*mm, 75*mm, 18*mm, 32*mm, 27*mm], repeatRows=1)
    ...
    doc.build(story)
    buffer.seek(0)
    safe_prn = re.sub(r"[^A-Za-z0-9_-]+", "_", str(student["prn"] or student["id"])).strip("_") or "student"
    return Response(buffer.getvalue(), mimetype="application/pdf",
                    headers={"Content-Disposition": f"attachment; filename=student_attendance_report_{safe_prn}.pdf"})
...
def add_subject():
    ...
    if request.method == "POST":
        ...
        if not code or not name or not year or not dept_id:
            ...
            return render_template("subject_form.html", subject=None, title="Add Subject", years=YEAR_OPTIONS)
        try:
    ...
    db.close(); return render_template("subject_form.html", subject=None, title="Add Subject", years=YEAR_OPTIONS)
    ...
    if request.method == "POST":
        ...
        if not code or not name or year not in YEAR_OPTIONS:
            ...
            try:
                ...
                if not isinstance(exc, sqlite3.IntegrityError) and not (POSTGRES_AVAILABLE and isinstance(exc, psycopg2.IntegrityError)): raise
                flash("Subject code already exists.", "error")
    departments=db.execute("SELECT * FROM departments WHERE active=1 ORDER BY name").fetchall(); db.close(); return render_template("subject_form.html", subject=subject, title="Edit Subject", years=YEAR_OPTIONS)
...
def delete_attendance_session():
    """Delete one complete attendance session safely on SQLite or PostgreSQL."""
    attendance_date=(request.form.get("attendance_date") or "").strip()
    session_type=(request.form.get("session_type") or "Lecture").strip()
    batch=(request.form.get("batch") or "").strip()
    lecture_time=(request.form.get("lecture_time") or "").strip()
    try:
        subject_id=int(request.form.get("subject_id") or 0)
        lecture_no=int(request.form.get("lecture_no") or 0)
    except (TypeError, ValueError):
        subject_id=0; lecture_no=0

    if not attendance_date or not subject_id or not lecture_no or session_type not in ("Lecture", "Practical"):
        flash("Invalid attendance session. Please try again.", "error")
    ...
    try:
        ...
        if not subject:
            db.rollback(); flash("Subject not found.", "error")
            return redirect(url_for("attendance_history"))

        where="subject_id=? AND attendance_date=? AND lecture_no=? AND COALESCE(session_type,'Lecture')=?"
        params=[subject_id,attendance_date,lecture_no,session_type]
        if session_type == "Practical":
            where += " AND COALESCE(batch,'')=?"
            params.append(batch)
        if lecture_time:
            where += " AND COALESCE(lecture_time,'')=?"
            params.append(lecture_time)

        count_row=db.execute(f"SELECT COUNT(*) c FROM attendance WHERE {where}", tuple(params)).fetchone()
        deleted=int(count_row["c"] if count_row else 0)
        if deleted == 0:
            db.rollback()
            flash("No matching attendance session was found. It may already be deleted.", "error")
            return redirect(url_for("attendance_history"))

        db.execute(f"DELETE FROM attendance WHERE {where}", tuple(params))
        db.commit()
        code=subject["code"] if subject else str(subject_id)
        log_activity("Attendance session deleted", f"{attendance_date} | {code} | {session_type} {batch} | Lecture {lecture_no} | {deleted} records")
        flash(f"Attendance session deleted successfully ({deleted} records).", "success")
    except Exception as exc:
        try: db.rollback()
        except Exception: pass
        app.logger.exception("Attendance session deletion failed")
        flash("Attendance could not be deleted. Please check the server log and try again.", "error")
    finally:
        try: db.close()
        except Exception: pass
    return redirect(url_for("attendance_history"))
...
def export_pdf():
    ...
    status_rows = db.execute(f"SELECT DISTINCT COALESCE(a.teacher_status,'Scheduled teacher present') teacher_status FROM attendance a WHERE {on_condition} ORDER BY teacher_status", on_params).fetchall()
    teacher_status_text = ", ".join(r["teacher_status"] for r in status_rows) or "—"
    buffer = io.BytesIO()
    ...
    info_data = [[
        ...
        Paragraph(f"<b>Teacher Status</b>&nbsp;&nbsp;: &nbsp;{xml_escape(teacher_status_text)}", meta_style),
    ],[
        Paragraph(f"<b>Session</b>&nbsp;&nbsp;: &nbsp;{xml_escape(session_type or 'All')}{(' / '+xml_escape(batch)) if batch else ''}", meta_style),
        Paragraph(f"<b>Course</b>&nbsp;&nbsp;: &nbsp;B.Pharm", meta_style),
        Paragraph(f"<b>Generated</b>&nbsp;&nbsp;: &nbsp;{datetime.now(ZoneInfo('Asia/Kolkata')).strftime('%d %b %Y | %I:%M %p')}", meta_style),
    ]]
...
def qr_attendance():
    db = get_db()
    try:
        subjects_all = teacher_or_admin_subjects(db)
        years = YEAR_OPTIONS[:]
        selected_year = (request.args.get('year') or request.form.get('year') or '1st Year').strip()
        ...
        if session_type != 'Practical':
            batch = ''
        elif batch not in ('Batch A', 'Batch B', 'Batch C', 'Batch D'):
            batch = ''
        ...
        start_time = (request.args.get('start_time') or request.form.get('start_time') or '').strip()
        end_time = (request.args.get('end_time') or request.form.get('end_time') or '').strip()
        teacher_status = (request.args.get('teacher_status') or request.form.get('teacher_status') or 'Scheduled teacher present').strip()
        allowed_status = (
            'Scheduled teacher present',
            'Scheduled teacher absent - substitute teacher',
            'Another teacher conducted the session'
        )
        if teacher_status not in allowed_status:
            teacher_status = 'Scheduled teacher present'
        conducted_by = request.args.get('conducted_by', type=int) or request.form.get('conducted_by', type=int)

        subjects = [x for x in subjects_all if x['year'] == selected_year]
        teachers = all_teacher_rows(db)

        # Find a timetable teacher when a matching slot exists. Timetable is optional
        # for QR creation so an unassigned timetable teacher cannot block QR generation.
        scheduled = None
        timetable_id = request.args.get('timetable_id', type=int) or request.form.get('timetable_id', type=int)
        if subject_id and start_time and end_time:
            day_name = date.fromisoformat(ad).strftime('%A')
            q = """SELECT t.*, COALESCE(u.full_name,u.username,'') teacher_name
                   FROM timetable t LEFT JOIN users u ON u.id=t.teacher_id
                   WHERE t.year=? AND t.subject_id=? AND t.day_of_week=?
                     AND COALESCE(t.session_type,'Lecture')=?
                     AND (COALESCE(t.batch,'')=? OR ?='Lecture')
                     AND t.start_time=? AND t.end_time=?
                   ORDER BY CASE WHEN t.teacher_id IS NULL THEN 1 ELSE 0 END, t.id LIMIT 1"""
            scheduled = db.execute(q, (selected_year, subject_id, day_name, session_type,
                                       batch if session_type == 'Practical' else '', session_type,
                                       start_time, end_time)).fetchone()
        if timetable_id and not scheduled:
            try:
                scheduled = db.execute("SELECT t.*, COALESCE(u.full_name,u.username,'') teacher_name FROM timetable t LEFT JOIN users u ON u.id=t.teacher_id WHERE t.id=? AND t.year=? AND t.subject_id=?", (timetable_id, selected_year, subject_id)).fetchone()
            except Exception:
                scheduled = None

        if request.method == 'POST':
            valid_subject = any(int(x['id']) == int(subject_id or 0) for x in subjects)
            if not valid_subject:
                flash('Please select a subject belonging to the selected year.', 'error')
            elif not start_time or not end_time or start_time >= end_time:
                flash('Please select a valid start and end time.', 'error')
            elif session_type == 'Practical' and batch not in ('Batch A', 'Batch B', 'Batch C', 'Batch D'):
                flash('Select a valid practical batch.', 'error')
            elif teacher_status != 'Scheduled teacher present' and not conducted_by:
                flash('Please select the actual / conducted-by teacher.', 'error')
            else:
                scheduled_teacher_id = scheduled['teacher_id'] if scheduled and scheduled['teacher_id'] else None
                if teacher_status == 'Scheduled teacher present':
                    if scheduled_teacher_id:
                        conducted_by = scheduled_teacher_id
                    elif not conducted_by:
                        # If timetable has no assigned teacher, the current staff member
                        # becomes the conducting teacher instead of blocking QR creation.
                        conducted_by = session['user_id']
                elif not conducted_by:
                    conducted_by = session['user_id']

                token = secrets.token_urlsafe(24)
                exp = datetime.utcnow().replace(microsecond=0) + __import__('datetime').timedelta(minutes=10)
                db.execute("""INSERT INTO qr_sessions(
                    token,subject_id,year,attendance_date,lecture_no,lecture_time,
                    start_time,end_time,session_type,batch,teacher_id,
                    scheduled_teacher_id,conducted_by_teacher_id,teacher_status,expires_at,active)
                    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)""",
                    (token, subject_id, selected_year, ad, 1, f'{start_time}-{end_time}',
                     start_time, end_time, session_type, batch, session['user_id'],
                     scheduled_teacher_id, conducted_by, teacher_status, exp.isoformat()))
                db.commit()
                db.close()
                return redirect(url_for('qr_attendance', token=token))

        token = request.args.get('token', '')
        qr = None
        qr_session = None
        if token:
            qr_session = db.execute("""SELECT q.*,s.code,s.name subject_name,
                COALESCE(st.full_name,st.username,'') scheduled_teacher_name,
                COALESCE(ct.full_name,ct.username,'') conducted_teacher_name
                FROM qr_sessions q JOIN subjects s ON s.id=q.subject_id
                LEFT JOIN users st ON st.id=q.scheduled_teacher_id
                LEFT JOIN users ct ON ct.id=q.conducted_by_teacher_id
                WHERE q.token=? AND q.active=1""", (token,)).fetchone()
            if qr_session:
                try:
                    if datetime.fromisoformat(qr_session['expires_at']) < datetime.utcnow():
                        qr_session = None
                except Exception:
                    qr_session = None
            if qr_session and QRCODE_AVAILABLE:
                import base64
                img = qrcode.make(request.url_root.rstrip('/') + url_for('qr_scan', token=token))
                bio = io.BytesIO(); img.save(bio, format='PNG')
                qr = 'data:image/png;base64,' + base64.b64encode(bio.getvalue()).decode()

        db.close()
        return render_template('qr_attendance.html', subjects=subjects, years=years,
                               attendance_date=ad, qr=qr, qr_session=qr_session,
                               token=token, batch=batch, session_type=session_type,
                               teachers=teachers, selected_year=selected_year,
                               selected_subject=subject_id, start_time=start_time,
                               end_time=end_time, teacher_status=teacher_status,
                               conducted_by=conducted_by, scheduled_teacher=scheduled)
    except Exception:
        try: db.rollback(); db.close()
        except Exception: pass
        app.logger.exception('QR attendance page error')
        flash('QR Attendance could not be loaded. Please check the selected details.', 'error')
        return redirect(url_for('qr_attendance'))
...
def qr_scan():
    ...
    st=current_student(db)
    if st['year']!=row['year'] or (row['session_type']=='Practical' and st['batch']!=row['batch']): db.close(); return render_template('qr_scan.html',valid=False,reason='This QR is not for your class or practical batch.')
    if request.method=='POST':
        db.execute("""INSERT INTO attendance(student_id,subject_id,attendance_date,lecture_no,lecture_time,start_time,end_time,session_type,batch,status,marked_by,academic_year,scheduled_teacher_id,conducted_by_teacher_id,teacher_status)
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            ON CONFLICT(student_id,subject_id,attendance_date,lecture_no) DO UPDATE SET lecture_time=excluded.lecture_time,start_time=excluded.start_time,end_time=excluded.end_time,session_type=excluded.session_type,batch=excluded.batch,status=excluded.status,marked_by=excluded.marked_by,academic_year=excluded.academic_year,scheduled_teacher_id=excluded.scheduled_teacher_id,conducted_by_teacher_id=excluded.conducted_by_teacher_id,teacher_status=excluded.teacher_status""",
