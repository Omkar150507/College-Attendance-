...
def students():
    ...
    db = get_db()
    if q:
        rows = db.execute("""SELECT s.*, u.username student_username
            FROM students s LEFT JOIN users u ON u.student_id=s.id AND u.role='student'
            WHERE s.prn LIKE ? OR s.name LIKE ? OR s.course LIKE ?
            ORDER BY s.name""", tuple(f"%{x}%" for x in (q, q, q))).fetchall()
    else:
        rows = db.execute("""SELECT s.*, u.username student_username
            FROM students s LEFT JOIN users u ON u.student_id=s.id AND u.role='student'
        ...
        for st in students:
            for n in range(1, lecture_count+1):
                ...
                if status in ("Present", "Absent"):
                    db.execute("""INSERT INTO attendance(student_id,subject_id,attendance_date,lecture_no,lecture_time,status,marked_by)
                        ...
                        (st["id"], selected_subject, selected_date, n, lecture_times[n], status, session["user_id"]))
        for n in range(1, lecture_count+1):
