...
def qr_attendance():
    ...
    try:
        ...
        candidates=timetable_candidates(db,selected_year,session_type,batch,subject_id,day_name) if subject_id else []
        teachers=all_teacher_rows(db)

        if request.method=='POST':
            tr=db.execute('SELECT * FROM timetable WHERE id=? AND year=? AND subject_id=? AND day_of_week=? AND COALESCE(session_type,\'Lecture\')=? AND COALESCE(batch,\'\')=?',(tid,selected_year,subject_id,day_name,session_type,batch)).fetchone() if tid and subject_id else None
            valid_subject=any(int(x['id'])==int(subject_id or 0) for x in subjects)
            if not valid_subject:
                flash('Please select a subject belonging to the selected year.','error')
            elif not tr:
                flash('Please select a valid timetable slot for the selected year, date, subject and session.','error')
        ...
        db.close()
        return render_template('qr_attendance.html',subjects=subjects,years=years,attendance_date=ad,qr=qr,qr_session=qr_session,token=token,batch=batch,session_type=session_type,candidates=candidates,teachers=teachers,selected_year=selected_year,selected_subject=subject_id,day_name=day_name)
    except Exception as exc:
