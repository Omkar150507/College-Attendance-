...
def student_my_report():
    ...
    rows = db.execute("""SELECT sub.code, sub.name subject_name,
        ...
        CASE WHEN COUNT(a.id)=0 THEN 0 ELSE ROUND(100.0*SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END)/COUNT(a.id),1) END pct
        FROM subjects sub LEFT JOIN attendance a ON a.subject_id=sub.id AND a.student_id=?
        WHERE sub.department_id=? GROUP BY sub.id ORDER BY sub.code""",
        (student["id"], student["department_id"])).fetchall()
    ...
    db.close()
    return render_template("student_report.html", student=student, rows=rows,
...
def student_my_report_pdf():
    ...
    styles = getSampleStyleSheet()
    title = ParagraphStyle("StudentReportTitle", parent=styles["Title"], fontSize=18, leading=22, alignment=TA_CENTER, textColor=colors.HexColor("#17324d"), spaceAfter=4)
    small = ParagraphStyle("StudentReportSmall", parent=styles["Normal"], fontSize=8.5, leading=11, alignment=TA_CENTER, textColor=colors.HexColor("#607789"))
    normal = ParagraphStyle("StudentReportNormal", parent=styles["Normal"], fontSize=9, leading=12, textColor=colors.HexColor("#263f53"))
    story=[]
    logo_path = BASE_DIR / "static" / LOGO_FILE
    if logo_path.exists():
        try:
            logo = Image(str(logo_path), width=22*mm, height=22*mm)
            head = Table([[logo, Paragraph("<b>Y.N.P. College of Pharmacy</b><br/><font size='8'>Student Attendance Management System</font>", normal)]], colWidths=[27*mm, 150*mm])
            head.setStyle(TableStyle([["VALIGN",(0,0),(-1,-1),"MIDDLE"],["BOTTOMPADDING",(0,0),(-1,-1),6]]))
            story.append(head)
        except Exception:
            story.append(Paragraph("Y.N.P. College of Pharmacy", title))
    else:
        story.append(Paragraph("Y.N.P. College of Pharmacy", title))
    story.append(Paragraph("STUDENT ATTENDANCE REPORT", title))
    story.append(Paragraph(f"Student: <b>{student['name']}</b> &nbsp;&nbsp; PRN: <b>{student['prn'] or '—'}</b> &nbsp;&nbsp; Year: <b>{student['year']}</b>", small))
    story.append(Spacer(1, 8))
    summary_data=[["Total Classes","Present","Absent","Overall Attendance"],[str(total),str(present),str(absent),f"{overall}%"]]
    st=Table(summary_data,colWidths=[42*mm]*4)
    st.setStyle(TableStyle([["BACKGROUND",(0,0),(-1,0),colors.HexColor("#eaf4fb")],["TEXTCOLOR",(0,0),(-1,0),colors.HexColor("#52708a")],["FONTNAME",(0,0),(-1,0),"Helvetica-Bold"],["FONTNAME",(0,1),(-1,1),"Helvetica-Bold"],["FONTSIZE",(0,0),(-1,-1),9],["ALIGN",(0,0),(-1,-1),"CENTER"],["BOX",(0,0),(-1,-1),0.6,colors.HexColor("#d6e4ef")],["INNERGRID",(0,0),(-1,-1),0.4,colors.HexColor("#e1ebf2")],["TOPPADDING",(0,0),(-1,-1),7],["BOTTOMPADDING",(0,0),(-1,-1),7]]))
    ...
    tbl=Table(data,colWidths=[10*mm,78*mm,22*mm,24*mm,22*mm,29*mm],repeatRows=1)
    tbl.setStyle(TableStyle([["BACKGROUND",(0,0),(-1,0),colors.HexColor("#1683e8")],["TEXTCOLOR",(0,0),(-1,0),colors.white],["FONTNAME",(0,0),(-1,0),"Helvetica-Bold"],["FONTSIZE",(0,0),(-1,-1),8.5],["ALIGN",(0,0),(0,-1),"CENTER"],["ALIGN",(2,1),(-1,-1),"CENTER"],["GRID",(0,0),(-1,-1),0.4,colors.HexColor("#dce7ef")],["ROWBACKGROUNDS",(0,1),(-1,-1),[colors.white,colors.HexColor("#f7fbfe")]], ["TOPPADDING",(0,0),(-1,-1),7],["BOTTOMPADDING",(0,0),(-1,-1),7]]))
    story.append(tbl); story.append(Spacer(1,10)); story.append(Paragraph("Generated by Y.N.P. College of Pharmacy Attendance Management System", small))
    doc.build(story)
    out.seek(0)
    safe_prn = re.sub(r"[^A-Za-z0-9_-]+", "_", str(student["prn"] or student["name"])).strip("_") or "student"
    return Response(out.getvalue(), mimetype="application/pdf", headers={"Content-Disposition": f"inline; filename=student_attendance_report_{safe_prn}.pdf"})
