...
def get_pg_pool():
    ...
    if PG_POOL is None:
        ...
        PG_POOL = ThreadedConnectionPool(1, 5, DATABASE_URL, sslmode=os.environ.get("DB_SSLMODE", "prefer"), connect_timeout=10, keepalives=1, keepalives_idle=30, keepalives_interval=10, keepalives_count=3)
    return PG_POOL
...
class CompatDB:
    def __init__(self):
        ...
        if self.postgres:
            ...
            self.conn = None
            # Render/PostgreSQL can close an idle pooled connection. Validate it
            # before handing it to a request and discard stale connections.
            last_error = None
            for _ in range(2):
                conn = None
                try:
                    ...
                    with conn.cursor() as health_cursor:
    ...
    def _reconnect(self):
        old = self.conn
        try:
            self.pool.putconn(old, close=True)
        except Exception:
            pass
        self.conn = self.pool.getconn()
        self.conn.autocommit = False
        with self.conn.cursor() as health_cursor:
            health_cursor.execute("SELECT 1")
    ...
    def execute(self, sql, params=()):
        if self.postgres:
            ...
            try:
                ...
                # A stale Render connection can fail on a harmless page load.
                # Retry read-only statements on a fresh connection. Never replay
                # writes automatically because a lost connection could have
                # committed the write on the server.
    ...
    def close(self):
        if self.postgres:
            try:
                # Never return a failed/dirty transaction to the pool.
                try:
                    self.conn.rollback()
                except Exception:
                    pass
