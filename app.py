...
def timetable_subject_teachers(db, year, session_type, batch, subject_id):
    """Return teachers assigned to this year/session/batch/subject, independent of weekday.
    QR attendance time is entered manually, so the selected attendance date does not
    have to be the same weekday as the timetable entry.
    """
    return db.execute("""SELECT DISTINCT u.id, COALESCE(u.full_name,u.username,'') teacher_name
        FROM timetable t JOIN users u ON u.id=t.teacher_id
        WHERE t.year=? AND t.subject_id=?
          AND COALESCE(t.session_type,'Lecture')=?
          AND (COALESCE(t.batch,'')=? OR ?='Lecture')
        ORDER BY teacher_name, u.id""",
        (year,subject_id,session_type,batch if session_type=='Practical' else '',session_type)).fetchall()
...
def qr_attendance():
    ...
    try:
        ...
        if selected_year not in YEAR_OPTIONS: selected_year='1st Year'
        session_type=(request.args.get('session_type') or request.form.get('session_type') or 'Lecture').strip()
        if session_type not in ('Lecture','Practical'): session_type='Lecture'
        batch=(request.args.get('batch') or request.form.get('batch') or '').strip()
        if session_type!='Practical': batch=''
        elif batch not in ('Batch A','Batch B','Batch C','Batch D'): batch=''
        ...
        if teacher_status not in ('Scheduled teacher present','Scheduled teacher absent - substitute teacher','Another teacher conducted the session'):
        ...
        subjects=[x for x in subjects if x['year']==selected_year]
        teachers=all_teacher_rows(db)
        scheduled_teachers=timetable_subject_teachers(db,selected_year,session_type,batch,subject_id) if subject_id else []
        scheduled_teacher_name=scheduled_teachers[0]['teacher_name'] if scheduled_teachers else ''
        ...
        if request.method=='POST':
            ...
            if not valid_subject:
                ...
                flash('No timetable teacher is assigned to this year, session, batch and subject. Please add the subject to Timetable first.','error')
            else:
                scheduled=scheduled_teachers[0]['id']
                if teacher_status=='Scheduled teacher present': conducted_by=scheduled
                ...
                db.execute("""INSERT INTO qr_sessions(token,subject_id,year,attendance_date,lecture_no,lecture_time,start_time,end_time,session_type,batch,teacher_id,scheduled_teacher_id,conducted_by_teacher_id,teacher_status,expires_at,active)
                    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)""",
                    (token,subject_id,selected_year,ad,1,f'{start_time}-{end_time}',start_time,end_time,session_type,batch,session['user_id'],scheduled,conducted_by,teacher_status,exp.isoformat()))
        ...
        db.close()
        return render_template('qr_attendance.html',subjects=subjects,years=years,attendance_date=ad,qr=qr,qr_session=qr_session,token=token,batch=batch,session_type=session_type,teachers=teachers,selected_year=selected_year,selected_subject=subject_id,scheduled_teacher_name=scheduled_teacher_name,has_scheduled_teacher=bool(scheduled_teachers))
    except Exception:
        db.rollback(); db.close()
        app.logger.exception('QR attendance page error')
        flash('QR Attendance could not be loaded. Please check the selected year, session, subject and timetable.','error')
