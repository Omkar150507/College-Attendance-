...
def delete_attendance_session():
    ...
    if session_type=='Practical':
        cur=db.execute("DELETE FROM attendance WHERE subject_id=? AND attendance_date=? AND lecture_no=? AND COALESCE(session_type,'Lecture')=? AND COALESCE(batch,'')=?",(subject_id,attendance_date,lecture_no,session_type,batch))
    else:
        cur=db.execute("DELETE FROM attendance WHERE subject_id=? AND attendance_date=? AND lecture_no=? AND COALESCE(session_type,'Lecture')=?",(subject_id,attendance_date,lecture_no,session_type))
